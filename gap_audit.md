...
- **Residual `burn`** (T1 re-verified at inner HEAD `c6b845f81` 2026-07-06, layered on prior `400c32624` snapshot):
  ...
  - `cargo tree -p kwavers-solver | grep burn` is **non-empty** at this HEAD: still pulls the full `burn v0.19.0` stack (`burn-core`, `burn-autodiff`, `burn-ndarray`, `burn-nn`, `burn-optim`, `burn-tensor`, `burn-ir`, `burn-derive`, `burn-common`, `burn-std`, `burn-backend`). The Batch #4 completion condition (`cargo tree -p kwavers-solver | grep burn` returns zero) is **unmet**.
  - **`burn::` line-hits total**: 186 across 80 files (`rg ' burn::' crates --type rust -c` at `c6b845f81`). 131 in `kwavers-solver`, 55 across top-level `kwavers` benches/examples/tests.
  - **`use burn` import-sites total**: 125 across 78 files (`rg 'use burn' crates --type rust -c`).
  - **PINN-subtree residual (131 hits / 64 files)**: at `crates/kwavers-solver/src/inverse/pinn/**`. Peer drain progress: `400c32624` slice 1 closed `burn_wave_equation_1d/{network,optimizer,physics,trainer,tests}`; `c6b845f81` slice 2 closed the `burn_wave_equation_2d` dependency graph (`acoustic_wave`, `cavitation_coupled`, `sonoluminescence_coupled`, `electromagnetic`, `adaptive_sampling`, `meta_learning`, `transfer_learning`, `distributed_training`, `quantization`, `uncertainty_quantification`, `universal_solver`, plus `field_surrogate/training/trainer.rs` partially). Residual unmigrated families: `burn_wave_equation_3d/{wavespeed,solver,optimizer,mod,tests}` (15+ hits), `pinn/elastic_2d/{training/{loop,optimizer/mappers,optimizer/pinn_optimizer,optimizer/tests,adaptive_sampling/batch},loss/pde_residual/tests}` (32+ hits, scale-locked to `kwavers-solver/src/inverse/pinn/elastic_2d/`), `pinn/ml/field_surrogate/{network,tests/training}` (5+ hits). Risk #8 framing now **partially-resolved** by `c6b845f81` (commit body affirms "per prior direction not to build burn-compat shims") — the `burn.rs` facade + `burn_compat` module remain on disk (referenced by the still-unmigrated families) but the peer's native-rewrite direction is now explicit and the slice-2 work removes two-thirds of the legacy surface. Risk #8 stays live until `burn.rs` / `burn_compat` are deleted and the `Cargo.toml:53` `burn` optional dep + `Cargo.toml:62-70` `pinn` `dep:burn` line are stripped.
  - **Top-level `kwavers` (55 hits / 17 files)**: `benches/{adaptive_sampling_opt, pinn_elastic_2d_training, pinn_vs_fdtd_benchmark}.rs` (3 files; `pinn_elastic_2d_training`: 26 hits, `pinn_vs_fdtd_benchmark`: 6 hits, `adaptive_sampling_opt`: 6 hits), `examples/{electromagnetic_simulation, field_surrogate_demo, multiphysics_sonoluminescence, pinn_2d_heterogeneous, pinn_2d_wave_equation, pinn_training_convergence, seismic_imaging_demo, seismic_imaging_3d_demo, skull_ct_phase_correction, transfer_learning_pinn}.rs` (10 files), `tests/{electromagnetic_validation, pinn_bc_validation, pinn_elastic_validation, pinn_ic_validation}.rs` (4 files).
