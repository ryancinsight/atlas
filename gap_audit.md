...
| `HELIOS-AEQ-MET-03` | delivery and portal dosimetry | `DeliveryFrame::leaf_fluence`, total delivered fluence, leaf width, ray step, and beam geometry distances were raw values; portal code typed fluence only internally before converting it back. | Helios | **RESOLVED; follow-up PR #32 implementation head `31147f0`, PM head `5832ffa` OPEN.** Portal fluence remains Aequitas `EnergyPerArea<T>` through Hyperion transmission; the branch pins merged Aequitas `e0fc5f3`. The implementation-head hosted run passed build, Rust, Python, deployment, and review checks but failed the replicated beam-transmission CPU benchmark (+0.08%/+0.21% at size 16384); the PM follow-up rerun is active, so no hosted-green result is claimed. |
...
- The current implementation state is: CFDrs' audited public physical carriers
  ...
  and thermal/perfusion metric gaps are closed on PR #324 head `c739c7b38`; PR
  #325's vessel-spacing slice is externally blocked by a conflicting branch and
  the peer Mnemosyne `TierSelection` dependency gap; and Helios PR #32's
  implementation-head hosted run passed build, Rust workspace, Python, and
  review checks but its replicated benchmark gate reported a beam-transmission
  CPU regression (+0.08%/+0.21%). Its PM follow-up head `5832ffa` is rerunning.
  These are delivery/verification residuals, not unimplemented Aequitas metric
  contracts. Each slice updates its child audit and uses its strongest value or
  analytical oracle before delivery.
...
- Kwavers PR #324 has valid local locked metadata and focused typed-metric
  evidence, but its current hosted head `c739c7b38` fails before compilation in
  several jobs because the CI checkout action still materializes the older Atlas
  graph at `806c6e7`; Cargo then attempts to rewrite `Cargo.lock` under
  `--locked`. The next consumer increment must repin the action to this graph
  revision and rerun the matrix. This is CI graph drift, not a missing metric.
