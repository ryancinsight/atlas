...
- **Residual `burn`** (T1 re-verified 2026-07-07 against the dirty inner `repos/kwavers` working tree after the Burn cleanup slice):
  - Kwavers manifests are clean: `rg -n "\bburn\b|burn-|Burn" -g Cargo.toml .` returns zero hits under `repos/kwavers`.
  - Kwavers direct Burn source imports are clean: `rg -n "use\s+burn::|burn::" crates xtask` returns zero kwavers source call-sites after excluding the literal audit-token string in `xtask/src/migration_audit.rs`.
  - The `crates/kwavers-solver/src/burn.rs` facade is absent and `rg -n "burn_compat|crate::burn|kwavers_solver::burn|pub mod burn|mod burn"` finds no `burn_compat` alias path. The requested 3-D path is native: no `BurnPINN3D*`, `BurnLossWeights3D`, `BurnTrainingMetrics3D`, or `burn_wave_equation_3d` source hits remain.
  - Burn-era identifier residual remains **416 lines across 71 files** (`BurnPINN*`/`BurnLoss*`/`BurnTraining*`/`burn_wave_*`/`burn_adapter` naming), concentrated in the older 1-D/2-D PINN public names and the solver beamforming adapter. This is naming/API debt, not a direct Burn crate dependency.
  - `cargo tree -p kwavers-solver --features pinn -i burn` remains non-empty through RITK provider crates (`ritk-image`, `ritk-interpolation`, `ritk-spatial`, `ritk-wgpu-compat`, and downstream `ritk-*` paths), so full Batch #4/RITK burn graph closure is still blocked outside the kwavers manifest surface.
  - Verification evidence: `rustup run nightly cargo check -p kwavers-solver --features pinn` passed. `rustup run nightly cargo nextest run -p kwavers --features pinn --test pinn_bc_validation --test pinn_ic_validation --status-level fail --no-fail-fast` compiled and ran 16 tests: 12 passed, 4 failed on legacy 3-D PINN loss thresholds (`test_ic_loss_zero_field`, `test_ic_combined_loss_decreases`, `test_bc_loss_decreases_with_training`, `test_dirichlet_bc_zero_boundary`). These are retained as validation residuals; assertions were not weakened.
...
| **apollo** | `.github/workflows/legacy-migration-audit.yml` | `cargo run -p xtask -- provider-audit` (native; hard-fails on forbidden ndarray references via `concat!("nd", "array")`) | (no `.allowlist` file — dynamic forbidden-pattern check + provider-usage matrix; consumes `xtask/src/provider_audit.rs` directly) | `[main, codex/**]` &ast; | per-submodule `9df5294e + 2940d66 + cd05eac` (workflow + branch-narrowing + workflow-YAML fix) |
...
&ast; Excludes `refactor/**` to defer day-1 verdict damage on Apollo's in-flight `refactor/apollo-fft-eunomia` migration (~234 dirty files mid-migration); expand to `refactor/**` once that migration lands, matching the cfdrs/ritk/kwavers shape.
