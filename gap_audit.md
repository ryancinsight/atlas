...
| `CFDRS-AEQ-MET-12` | `cfd-1d` channel cross-sections, channel geometry, and network edge properties | `CrossSection`, `ChannelGeometry`, `Edge`, and `EdgeProperties` still expose channel dimensions, area, hydraulic diameter, and length as raw SI scalars. | CFDrs, Aequitas | **OPEN; next implementation slice.** Acceptance is typed `Length`/`Area` geometry through the public network boundary, scalar extraction only at resistance and solver kernels, migrated constructors/tests, and focused value-semantic gates. |
...
- The current implementation state is: CFDrs' audited report carriers are
  ...
  `bc037a79`, and component geometry/volume metrics in `670cd96e`; the fresh
  network cross-section and edge-property boundary is tracked as MET-12 above;
