...
Helios, or Kwavers public surfaces. The CFDrs network slice, Helios physical
metric slices, and Kwavers transducer/driver slices now pass their applicable
consumer gates. Remaining diagnostics are external shared-provider warnings
(Leto dead code and Windows linker/build-script warnings); they are not metric
contract failures and are not masked by consumer-side compatibility shims.
...
- **Kwavers:** the workspace Coeus paths now target `../coeus/crates/*`.
  Full locked workspace check passes; driver Nextest passes 487/487;
  transducer/boundary Nextest passes 318/318 with 1 skipped; warning-denied
  Clippy and doctests pass for the touched package surfaces.
- **Helios:** the Coeus patch paths now target `../coeus/crates/*`.
  Workspace check, warning-denied Clippy, and doctests pass; focused
  domain/simulation Nextest passes 72/72. The helical base-SI regression proves
  a 10 mm couch advance per rotation.
