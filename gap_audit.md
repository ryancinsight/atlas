...
CFDrs PR #349 source head `3fbffc6a` carries the bounded `cfd-core`/`cfd-math`
...
254 `cfd-math` test-target errors; commit `3fbffc6a` closes the 43-error
`linear_solver/block_preconditioner.rs` family with invariant-bearing
expectations and no production-path change. Earlier source cleanup
...
`3fbffc6a` own the `cfd-core`/`cfd-math`
