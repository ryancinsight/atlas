...
CFDrs PR #349 source head `22d74042` passes the focused local value-semantic
...
lint rewrite is added to this PR. Commit `22d74042` owns the `cfd-core` test
unwrap and GPU-test print classes without touching the peer Cargo.lock. Local
...
Provider PR #349 source head `22d74042` is open. Hosted book figures pass, but
...
debt rather than a solver regression. Commit `22d74042` owns two reported
classes without touching the lockfile. The exact-head hosted rerun must
