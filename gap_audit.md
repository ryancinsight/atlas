...
- **2026-07-08 — `ndarray` → `leto's ndarray-compat` migration tracking entry**: per fresh T1 verification via `rg -n 'ndarray' crates --type rust` at inner HEAD `35ee01076` (branch `codex/kwavers-core-moirai-parallel`):
  ...
  - **2026-07-08 `apply_acoustic_freq` test-mock ndarray slip (surfaced this session via `cargo nextest run --workspace --lib`)**: T1 verification at kwavers inner HEAD `ccc6bbf9e6` (`Workspace-wide ndarray↔leto boundary fixes; cargo check --workspace passes`) shows the bulk ndarray→leto migration commits this session closed 5 commits deep on the inner `codex/kwavers-core-moirai-parallel` branch (`ccc6bbf9e6` + `4eb003c73` + `44d958462` + `5a2ad402c` + `59e838cf4` + `09c645f30`), and `cargo check -p kwavers-solver --workspace` succeeds with only 1 cosmetic dead-code warning (`fn to_leto3` unused in `crates/kwavers-simulation/src/dispatch/elastic_pstd.rs:8:4`). However, `cargo nextest run --workspace --lib` fails to compile 1 site at `crates/kwavers-solver/src/plugin/mod.rs:202-209` with `error[E0053]: method apply_acoustic_freq has an incompatible type for trait`: the test-mock `NullBoundary::apply_acoustic_freq` signature reads `_field: &mut Array3<kwavers_math::fft::Complex64>` (resolving via the in-scope `use ndarray::Array3;` import at line 182 — which shadows the workspace's `leto::Array3`'d naming convention); the `Boundary` trait now declares `_field: &mut leto::Array<eunomia::Complex<f64>, VecStorage<eunomia::Complex<f64>>, 3>` (per the trait rewire at `crates/kwavers-solver/src/.../plugin/mod.rs:204:21`). A single-line fix is `use ndarray::Array3;` → `use leto::Array3;` at line 182. Note a second site at `crates/kwavers-solver/src/forward/pstd/physics/residual_gas_absorption.rs:74` (`spectrum: &mut Array3<kwavers_math::fft::Complex64>`) ALREADY uses `use leto::{Array3, ArrayView3};` (line 65), so its `Array3` resolves correctly to `leto::Array3`; the kwavers-solver forward module compiles clean. Only the `plugin/mod.rs` test mock is broken — kwavers-solver compiles in `--no-run` mode but cannot produce a test binary. **Peer-owned (per `concurrent_agents` disjoint-scope rule)**: atlas-meta records the residual; the inner peer stream owns the `crates/kwavers-solver/src/plugin/mod.rs:182,204` 2-line edit. **Verification evidence (this turn)**:
    ...
    - ritk `cargo nextest run -p ritk-python --lib` PASSES 47/47 (1m 41s compile)
