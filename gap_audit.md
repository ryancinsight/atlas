...
- The remaining provider semantic gaps are volumetric energy density
  ...
  scaled units required by their consumer contracts.
...
| `KWAVERS-AEQ-MET-03` | transducer frequency, geometry, and Rayleigh models | Frequency response, element dimensions/area/volume, propagation range, wavelength, attenuation, and acoustic impedance cross public APIs as `f64`. | Kwavers | **PARTIAL; PR #324 OPEN.** Commit `65f5a200` types frequency response, element geometry, area/volume, and design contracts; Rayleigh/material residuals remain. Hosted `recurseml/analysis` errors; dimensionless Q, bandwidth fraction, directivity, and reflection coefficients remain scalar. |
...
- The current implementation state is: Kwavers transducer contracts are
  partial on PR #324; the CFDrs report carrier is implemented on PR #315; the
