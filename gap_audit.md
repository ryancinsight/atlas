...
## Finding 2026-08-20: Leto domain book delivery

The Leto default at `a2bc13f` had no `docs/book/` or Pages caller, and the
fetched `codex/leto-book` branch was only a skeleton with eight placeholder
chapters. Provider PR [#119](https://github.com/ryancinsight/leto/pull/119)
now carries exact head `aaa823895bfbb8121f1f435abfb7c6bf6a48245c`: nine
...
Atlas root `04be0d6` pass for the 22 registered providers and report all
...
The intervening Mnemosyne default movement was reconciled by the root-only
gitlink commit `04be0d6`, advancing `638ddab8` to `5fd08df6`. The provider
checkout was already at the fetched commit, and no provider working-tree files
were staged. The post-reconciliation exact-head audit is clean.
...
CFDrs source commits `7be6727b` and `2721539e` add the `cfd-python` abi3-py38
release caller, pin both the shared workflow and exact Atlas graph to
`5936303`, derive the Python module version from `CARGO_PKG_VERSION`, add
installed-wheel tests for the Casson constitutive equation, Womersley alpha,
and analytical Poiseuille values, and exclude generated simulation output and
Python bytecode from the sdist. Draft PR #355 is open; hosted Rust and book
jobs are queued, while the external RecurseML status is report-only.

Local evidence: provider formatting, workflow YAML parsing, Python test syntax,
and `maturin sdist` pass; the generated `cfd_python-0.3.0.tar.gz` is 68.61 MiB
and contains zero forbidden output/cache members. `cargo check --locked
-p cfd-python` stops before compilation because the shared Atlas overlay
requests lockfile changes and `--locked` rejects them. This is a
resolver/overlay blocker, not source correctness evidence. PR merge and
exact-head hosted results remain open.
...
declare their real `aequitas` and `proteus` external crates; the three focused
API fragments remain intentionally non-standalone.

Local Rust format, Clippy, package build, nextest (20/20), doctest (1/1),
...
`32341399588`, MSRV `32341399599`, and Deploy mdBook `32341400004` pass. Atlas
records the merged default; live Pages returns HTTP 200 with the expected
Mnemosyne title and the Miri-backed post-merge gate is green.
