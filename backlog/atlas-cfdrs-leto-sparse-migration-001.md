...
outcome: replace `crates/cfd-math/src/linear_solver/direct_solver.rs` body
with calls to `leto_ops::application::sparse::SparseLuSolver::solve_view` on
real CSC-typed inputs; remove any `with_direct_threshold(512)` regime that
exists only to route medium saddle-point FEM matrices to GMRES.
...
- Refs: backlog.md#ATLAS-LETO-OPS-SPARSE-LU-001 (closed), leto origin/main
  `687b670`.
