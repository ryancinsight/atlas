...
- **open:** on `codex/helios-book-figures-closeout` (`e66a16afcd7`): `cargo fmt -p xtask` + amend; drop `--locked` from the maturin and `cargo bench --no-run` CI steps. Re-push, open follow-up PR, confirm the `SSOT_IN_SYNC` log capture.
