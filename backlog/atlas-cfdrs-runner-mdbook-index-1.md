...
- (b) closed: curl pre-step materializing `INDEX.md` landed in `book-pages.yml`; end-to-end emulation passes.
...
- next (c), depends on (a)+(b): a re-run must produce the verbatim `DRIFT_DOCS_NOT_IN_SPECS: N ...` line at `Check book figures`, closing `ATLAS-CHECK-FIGURES-CI-VERIFY-DEFER` (also the depends-on/follows link).
