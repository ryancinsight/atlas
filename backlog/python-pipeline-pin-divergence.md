...
integrator: root; branch: ci/atlas-python-wheel-verification; last-update: 2026-09-20
...
verification limit: release callers still attach artifacts only on a published tag; `verification=true` now reuses the matrix and metadata validator without release authority or registry upload. Current increment (2026-09-20) derives one version across wheels and sdist, skips release attachment, and is covered by Atlas workflow contract tests; a downstream caller must adopt it before hosted evidence is claimed.
