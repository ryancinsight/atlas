...
| <a id="ADR-0038"></a>**0038** | One generic `ComputeBackend` conformance suite in a new `hephaestus-conformance` workspace crate — 15 939 lines of per-backend `contract.rs` with only 5 test names common to all four backends (cuda/wgpu share 87 by copy-paste; rocm/wgpu share 7; metal held to 40 assertions vs wgpu's 130); triage into contract / capability-gated / backend-intrinsic; suite generic over `<B: ComputeBackend, T: Scalar>`; Leto runs it as the CPU reference — **delivered 2026-09-10**: `hephaestus-conformance` (7 020 LOC, generic over `ComputeDevice`, dev-dep of **five** crates — the four accelerators, called from 18–19 test files each, plus `hephaestus-host`, which runs 2 of 22 clauses and implements one of the nineteen seams behind them); residual is draining 16 391 lines of legacy per-backend `contract.rs` | Accepted | 2026-07-28 | `[arch]` `[minor]` | hephaestus / leto / atlas-meta | gpu-substrate |
