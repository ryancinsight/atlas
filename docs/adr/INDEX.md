...
| <a id="ADR-0006"></a>**0006** | `eunomia::ComplexField` as `kwavers-math` `CsrScalar` SSOT (CR-EUNOMIA-COMPLEX); sets the per-batch `num_complex::Complex<T>` migration convention | Accepted | 2026-07-05 | `[minor]` | kwavers-math | numeric-ssot |
...
| <a id="ADR-0031"></a>**0031** | Real sparse LU factorization for `leto-ops::SparseLuSolver` — replace dense-backed misnamed LU with CSC-based sparse LU + partial pivoting; natural column ordering by default, AMD ordering delivered 2026-08-06 (`amd.rs`, opt-in via `OrderingStrategy::AmdApproxMinDegree`; RCM still unimplemented) | Accepted | 2026-07-23 | `[arch]` `[minor]` | leto / CFDrs / atlas-meta | upstream-algorithm |
...
| <a id="ADR-0039"></a>**0039** | Compute-substrate topology across Apollo, Leto, Hephaestus, and Coeus — Hephaestus owns the vendor dimension and no consumer re-forks it; seam coverage precedes consumer collapse; the Leto/Hephaestus decomposition pair gets one role trait and one differential oracle; Apollo's repeated plan/execution scaffold consolidates behind a generic layer — **re-measured 2026-09-10**: all four `coeus-<vendor>` crates are now thin bindings over `hephaestus-<vendor>` (rocm 152 / metal 163 lines of `src`), so the vendor dimension is **not** re-forked; the 2026-08-12 Metal/ROCm provider collapse is real. Open residual is the item the ADR's own revision names: CUDA/WGPU duplication (`coeus-cuda` 2 204 / `coeus-wgpu` 2 356 lines of `src`) plus the Apollo/Leto follow-up. Separately, 10 837 lines of coeus backend **test** bodies are hand-rolled and consume neither `hephaestus-conformance` nor any shared suite | Accepted | 2026-07-28 | `[arch]` | coeus / hephaestus / leto / apollo / atlas-meta | gpu-substrate |
| <a id="ADR-0038"></a>**0038** | Shared backend contract suite; delivered suite ownership, with consumer consolidation and host coverage tracked by [the coverage item](../../backlog/atlas-hephaestus-host-seam-coverage.md). Numerical measurements in the ADR describe the 2026-09-10 snapshot. | Accepted | 2026-07-28 | `[arch]` `[minor]` | hephaestus / leto / atlas-meta | gpu-substrate |
...
The [generated ADR index](README.md) lists the current authored sequence and statuses.
...
| <a id="ADR-0019"></a>**0019** | TREE-DUP-002: Dual channel consolidation — fold `unified_channel` into `channel/` module, extend `Channel<T>` trait with batching/close/stats, merge error types, migrate sole consumer `moirai-iter` | Accepted | 2026-07-16 | `[major]` | moirai-core | module-hierarchy |
| <a id="ADR-0055"></a>**0055** | continuum domain decomposition axis: a domain is a conserved quantity + balance operator + constitutive closure; `proteus` closes, `ares` balances solid momentum, `prometheus` balances species mass; seven checkable boundary rules + first-party substrate contract + typed-physics quantity ledger; records the electromagnetics (`astrape`) and parametric-CAD (`daedalus`) gaps without promoting either | Accepted | 2026-09-03 | `[arch]` | proteus (provider); CFDrs / kwavers (consumers); ares / prometheus (candidates) | domain-boundary |
