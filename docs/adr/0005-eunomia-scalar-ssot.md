...
- Status: **Accepted** — implementation **closed** 2026-07-05 (eunomia `57d7789` + coeus `2b3f820` + leto `b15439b` rebind + atlas-meta closure `5328de1c`). Pre-1.0 breaking trait rebind, version bump `0.35.1 → 0.36.0` on leto workspace. RG-verified zero residual `Scalar::add/sub/mul/div/ZERO/ONE/bitand/bitor/bitxor/count_ones/to_f64` UFCS references in `crates/`. Pre-merge gate passed green on `codex/leto-cr4-ssot-rebind`: `cargo nextest run -p leto-ops` 270/270 + `-p leto` 189/189 + 8 doctests + clippy `-D warnings` green on `--lib --tests` scope (`--all-targets` blocked on peer `hermes-simd-intrinsics` `is_runtime_supported` gap, NOT a CR-4 regression — documented in atlas-meta gap_audit). Signed off under `interaction_policy` autonomy mode on 2026-07-04: the change is internally verifiable, a local refactor of provider-internal vocabulary with provider-only land; no security/privacy/permissions/data-loss dimension present; no external `coeus-core` consumer of the seven deleted trait methods existed in atlas per T1 grep at sign-off time.
- Date: 2026-07-04.
