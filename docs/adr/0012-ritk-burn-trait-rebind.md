...
| #3.g | `ritk-python` + `ritk-cli` + `ritk-snap` | — | **DEFERRED to sub-batch #5 (2026-07-07, see amendment below)** | n/a |

**Amendment (2026-07-07)**: sub-batch #3.g is deferred to sub-batch #5, not landed as a per-crate test port. Investigation found the row 121 premise does not hold for these three crates: unlike `.a`-`.f` (pure-algorithm crates with test-only Burn usage, portable via an additive Coeus/Moirai-typed sibling struct + single-test-file rewrite), `ritk-cli`/`ritk-snap`/`ritk-python` are the application/binding layer built directly atop `ritk_image::tensor::Backend` — which is literally `pub use burn::tensor::backend::Backend` (`ritk-image/src/lib.rs:31,36`), not an abstraction over it. `ritk-snap`'s `ViewerCore<B: ritk_image::tensor::Backend, const D: usize>` (`ritk-snap/src/viewer.rs:138`) and `FilterKind::apply_filter` dispatch through ~20 Burn-keyed filter variants directly in the running GUI app's filter-apply path (`ritk-snap/src/app/filter.rs`); `ritk-python/src/image.rs` pattern-matches Burn's `NdArrayTensor::F32` tensor-primitive internals directly in its PyO3 binding surface. The only isolated test file in these three crates (`ritk-snap/src/app/tests/rt.rs`, 8 tests) exercises `ViewerCore`/`FilterKind` — production dispatch code, not a leaf algorithm — so a faithful port would require building a parallel `ViewerCore`/`FilterKind` twin (bound on `ComputeBackend` instead of Burn's `Backend`) covering the full filter-variant surface: an application-scale addition, not a "one test module" sub-atomic commit, and one that sub-batch #5's backend flip would very likely make throwaway shortly after (violates the subtractive-bias / optimization-farsight preference against building work that gets discarded). Decision (user-confirmed): `ritk-cli`, `ritk-snap`, and `ritk-python` convert directly during sub-batch #5's full `burn`/`burn-ndarray` Cargo strip + `Image<B, D>` re-export switch, rather than getting an interim parallel-typed surface. Sub-batch #3's per-crate queue is closed at `.f`; the sub-batch #3 closeout condition below is updated accordingly. **Naming note (2026-07-07)**: per user direction, new components introduced in this migration should not carry an "Atlas" name prefix going forward (e.g. name by domain concept, not by migration wave) — the already-shipped `Atlas*` types in sub-batches `.a`-`.f` are unaffected pending a separate, explicitly-scoped rename decision.

The sub-batch #3 closeout is when the last landed per-crate commit (`#3.f`) is verified and `xtask/burn_surface.allowlist` source-entries parcel to the migration-done rows per sub-batch #6's refresh ritual for the six converted crates (`ritk-filter`, `ritk-registration`, `ritk-segmentation`, `ritk-model`, `ritk-statistics`, `ritk-io`/`ritk-interpolation`/`ritk-transform`); `ritk-cli`/`ritk-snap`/`ritk-python` remain on the Burn-keyed surface until sub-batch #5.
