...
2. 🟡 Item (a) lands on kwavers peer stream: `cd /d/atlas/repos/kwavers && git log --oneline -1 949e5a39..HEAD` enumerates the flush commit; the file-touch count matches the corrected 30-file drift surface (re-probed 2026-07-09: actual count is 30 modified files with 30/30 whitespace-only deltas — see `D:/atlas/backlog.md` §Blocker-triage chore briefs row 4 — the prior 1,315-file estimate in this ADR's prior edits was overstated by ~44×).
...
This ADR does NOT address — and the closeout-tag ceremony does NOT include — the following carried-forward pre-existing blockers (per ADR 0013 §Out of scope items 1-5 with 2026-07-09 re-probe corrections applied per `atlas/backlog.md` §Blocker-triage chore briefs):

1. **`repos/ritk/crates/ritk-wgpu-compat/Cargo.toml` burn workspace-manifest** — per ADR 0012 §Sub-batch #5 major Burn remove cycle. **➜ RETIRED 2026-07-09 (re-probe)**: ritk peer `cargo check -p ritk-wgpu-compat --lib --no-default-features` rc=0 verified cleanly (17.32s); chore briefs row 1 archives the entry. No ceremony impact.
2. **`repos/ritk/crates/ritk-registration/Cargo.toml` burn dep strip** — per ADR 0012 §Sub-batch #5 `[major]`. **➜ RECLASSIFIED 2026-07-09 (re-probe)**: actual cause is `direct-parzen` feature-gate regression (2 E0432 errors at `image_cache_helpers.rs:7:43` + `metric/histogram/mod.rs:10:17` — not a Burn dep strip); chore briefs row 2 files this as a discrete chore on ritk peer stream for the feature-gate corrective commit.
3. **`repos/ritk/crates/ritk-image` autodiff-module syntax** — per ADR 0012 §Sub-batch #3 per-crate test ports (excluded per the 2026-07-07 amendment; deferred to sub-batch #5). **➜ RETIRED 2026-07-09 (re-probe)**: ritk peer `cargo check -p ritk-image --lib --no-default-features` rc=0 verified cleanly (18.77s); autodiff references are inert feature-gated test fixtures. Chore briefs row 3 archives the entry.
4. **1,315-file mechanical working-tree drift on `repos/kwavers`** — item (a)'s flush commit IS the resolution mechanism for this item; once item (a) lands, this carryover is retired. **➜ CORRECTED 2026-07-09 (re-probe)**: actual drift count is 30 modified files (sample-audit shows 30/30 modifications are whitespace-only ` LF will be replaced by CRLF` normalization), not 1,315 — carried-forward estimate was overstated by ~44×. Item (a)'s flush commit on kwavers peer stream should reflect the corrected 30-file count in its commit body for accuracy.
5. **`kwavers-math` Phase-3 / Phase-4 ndarray → leto array migration breakage (post-slice-9)** — per ADR 0013 §Failure modes / risks line 1; per `atlas/backlog.md` ##In-flight claims row; the kwavers peer stream's continuing Phase-3 / Phase-4 workstream is its own orthogonal carve-out. The item (b) predicate unification is narrowly the SLICE 7 divergence.rs predicate unification; it does NOT block on the broader Phase-4 ndarray → leto workstream. **➜ CONVERTED 2026-07-09 (re-probe)**: 2 actual commits have landed on kwavers peer stream post-slice-9 — `445ab9b2a` `fix(kwavers-math): linear algebra import/API mismatches` + `e2e1e180f` `fix(kwavers-math): grid/transducer compilation issues` — both kwavers-math-scoped exclusively. Chore briefs row 5 files the continuing workstream as a discrete chore on kwavers peer stream.
