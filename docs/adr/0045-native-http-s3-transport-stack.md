...
| P4 | Implementation delivered; hosted measurement pending | `consus/crates/consus-zarr/benches/s3_rusoto_moirai.rs` adds a Criterion comparison of identical ranged reads through `S3MoiraiReader` and `S3Reader`; the `s3-minio` CI job runs it against a deterministic 1 MiB object and uploads `target/criterion/s3_range_read`. Local formatting passes; the MinIO run is the authoritative comparative result and has not yet been collected. |
| P5 | Open | Both backends coexist and **neither is default**: `consus-io` and `consus-zarr` both declare `default = ["std"]`, and the consus facade exposes no S3 passthrough at all. `rusoto_core`, `rusoto_s3`, `reqwest`, and `tokio` remain optional dependencies of the legacy feature, not demoted. |
...
Criterion report is performance evidence, not a correctness substitute. P5
remains open until the hosted artifact is collected and the default-backend
contract is separately respecified.
...
- consus S3: SigV4 known-answer tests from AWS's published vectors; a
  ...
  in CI.
- Comparative benchmarks are the go/no-go evidence for P5's default flip and are
  **outstanding**. An intermediate regression there triggers profiling and
  optimization, not reversion.
