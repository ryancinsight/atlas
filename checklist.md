...
- This session (2026-07-06, codex) performed Atlas-meta cleanups and the RITK Burn GPU-default closeout only: deleted stray `nul` artifact, retracted stale surfacing risks #2/#3, closed #7, closed RITK risk #1, and refreshed the submodule-WIP inventory. Did NOT migrate kwavers/coeus/gaia source because those scopes are peer-active. The peer's next atomic commits to `codex/kwavers-core-moirai-parallel` continue the Batch #1 surface; Coeus and Gaia must land their source+PM changes inside their own repos before Atlas-parent pointer work.
