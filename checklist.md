...
**Progress (slice 1, peer 2026-07-06)**: peer landed inner commit `400c32624` "Migrate burn_wave_equation_1d PINN to native coeus" on `codex/kwavers-core-moirai-parallel`. 12-file slice covering `crates/kwavers-solver/src/inverse/pinn/{beamforming/burn_adapter.rs, ml/burn_wave_equation_1d/{network,optimizer,physics,trainer,tests}/*, ml/{validation, burn_wave_equation_1d/tests}.rs}` rewritten against `coeus_nn::Linear`, `coeus_autograd` free functions, `coeus_optim::SGD`. Continued use of the `crates/kwavers-solver/src/burn.rs` shim facade + `burn_compat` module permits the remaining 126 PINN-subtree + 17 top-level files to keep importing `burn::*` without source rewrites per slice — see `gap_audit.md` surfacing risk #8. Residual inventory refreshed in `gap_audit.md` §kwavers (T1 evidence at `400c32624`): 315 `burn::` line-hits / 144 files; 222 `use burn` import-sites / 139 files.
...
| #3 | `[minor]` | ritk Burn-keyed trait rebind | `ritk/atlas-migration-push/batch3` | 2026-07-05 | reserved; `repos/ritk` `main` is now advanced to `d7a940b5` after the Burn GPU-default cleanup at `65a1a0fd` and the Batch #3 sub-batch #1 Atlas-typed parallel trait surface. Closure still requires current package gates for the remaining rebind packages; the Burn GPU-default drift is closed separately. |
...
- This turn (2026-07-06, codex) surfaced the Batch #4 slice-1 partial-land + the burn.rs facade `integrity` concern via atomic atlas-meta doc edits only (PM artifacts at workspace root). Did NOT migrate kwavers/coeus/gaia source because those scopes are peer-active.
- Awaiting the peer's slice-2..N sequence on `codex/kwavers-core-moirai-parallel` covering: `universal_solver` → `burn_wave_equation_3d/{wavespeed,tests}` → `cavitation_coupled` → `meta_learning/{gradient,learner,optimizer}` → `sonoluminescence_coupled` → `uncertainty_quantification` → `transfer_learning` → `adaptive_sampling` → `quantization` → `acoustic_wave` → `field_surrogate` → `elastic_2d` → 17 top-level `kwavers/{benches,examples,tests}` files → `burn.rs`+`burn_compat` deletion → `kwavers-solver/Cargo.toml:53` `burn` removal + `kwavers/Cargo.toml:138` `burn` dev-dep removal + `pinn` feature `dep:burn` line strip (L62-70). The peer's `400c32624` commit claims "rewritten directly against coeus rather than via a burn-shaped compat facade"; resolution requires either that claim's retraction or the facade deletion.
