...
### Sub-batches #2-#6 — RESERVED (#2 closed 2026-07-06; #3-#6 reserved)
...
- **Latest closed migration batch**: Batch #3 sub-batch #1 (RITK Burn-keyed trait rebind, additive Atlas-typed parallel trait surface; landed in inner RITK at commit `d7a940b5812ff8ac44094096cd581131f3eec175`, 8 files, +158 lines; pointer advanced by atlas-meta commit `61931faf` 2026-07-06). ADR 0012 `ritk-burn-trait-rebind` authored in `docs/adr/`. The remaining Batch #3 sub-batches #2-#6 remain reserved per the atomic-boundary discipline. Earlier closed batch: Batch #2 (CFDrs nalgebra → leto + nalgebra-sparse → leto-ops `CsrMatrix`) — landed in inner CFDrs as `d58d1fe3` on branch `codex/cfdrs-atlas-migration` (2026-07-05 23:33); Atlas-parent submodule pointer advance landed as `51922a56` (peer 2026-07-06 08:14).
...
- Next claim: observation-mode; both `kwavers` Batch #1 (Rayon→Moirai, 84 `.par_for_each` sites / 28 files remain) and Batch #4 (Burn→Coeus, 186 `burn::` hits / 80 files remain after slice-2 drain, slices 3..N pending: `burn_wave_equation_3d`, `elastic_2d`, top-level benches/examples/tests, plus facade + Cargo.toml strip) are peer-active. Atlas-meta remains ready to bump the `repos/kwavers` submodule pointer only when (a) Batch #1 closure lands cleanly AND (b) risk #8 fully resolves (peer deletes `burn.rs` + `burn_compat` and strips `Cargo.toml:53` `burn` dep + `pinn` `dep:burn` line).
