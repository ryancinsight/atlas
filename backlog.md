...
| US-023-A1 | Implement the ADR 0042 seam in `ritk-image`: `CoordinateMap` with `Cartesian` + `CurvilinearArray`, carried on `Image`, dispatched by both batch and both single-point transforms. | [major] | done 2026-08-13 — ritk PR #128 merged as `c608f758` | Claude | Met. Cartesian path bit-identical (pinned by test); curvilinear round-trip, fan symmetry/curvature, out-of-fan NaN, dimensionality rejection all covered. 1173 tests, clippy `-D warnings`, rustdoc, fmt clean. Merge held per the `[major]` review rule — `Image::into_parts` is a breaking signature change |
| US-023-A2 | `PhasedArray3D` variant on the ADR 0042 seam. | [minor] | **review** 2026-08-13 — ritk PR #131, commit `9c29e9ff` | Claude | Met. Boresight, per-angle independent steering, round-trip, out-of-volume NaN, degenerate-steering rejection, and `D == 3` enforcement all covered. 1185 tests, clippy `-D warnings`, rustdoc, fmt clean |
...
| US-023-A3 | Migrate kwavers `b_mode/scan_conversion.rs` onto the seam as an ordinary resample and **delete** the standalone converter; inverse scan conversion falls out as the opposite direction. | [minor] | todo — unblocked by #128 | — | Differential against the current `ScanConverter` within bilinear interpolation error, then that converter and its callers are gone |
...
- Apollo PR #86 was rebased against current default and both rename commits
  were shown to be superseded by Apollo `49632c6c` (ADR 0039 s5), which
  already dissolves the affected `helpers.rs` modules into concern-specific
  leaves. The rebased branch had no diff after dropping the duplicate work.
- The stale PR was closed with the exact evidence and its remote branch was
  deleted; current default `fc5648964c8194447ef5deea43a8aa9c0dae7c63` passes
  post-merge Apollo CI `31708720285`.
...
- Owner: book documentation only across eight provider repos; no source,
  manifest, or gitlink changes.
- Replaced all 64 placeholder ("Chapter prose deferred") chapters across
  Themis (8), Mnemosyne (10), Consus (4), Moirai (10), Hephaestus (10),
  Coeus (10), Apollo (10), and Iris (2) with API-accurate prose.
- Each chapter covers: key public types, design patterns, code examples
  (rust,ignore), and Atlas stack position.
- Evidence: link detector 0/0/0 for all 8 books; mdBook builds pass; all
  8 delivery commits touch 0 version-bearing lines; version-guard coherence
  clean (235/215/1037, 0 defects); provider integration audit OK.
- Branches pushed: codex/themis-book-closure (`7a6744b`),
  codex/mnemosyne-book-closure-2 (`fdd8bd6`), codex/consus-book-closure
  (`a21228a`), codex/moirai-book-closure (`31816dc`), codex/hephaestus-book-closure
  (`ae657fc`), codex/coeus-book-closure (`915f5ea0`), codex/apollo-book-closure
  (`3fd88f5e`), codex/iris-book-closure (`2335464`).
- Remaining: owner PR merge for each branch; root gitlinks stay at merged
  heads pending those merges.

## ATLAS-CASCADE-ALIGNMENT-001 — Consumer alignment for the 0.42/0.5/0.26/0.19 provider cascade [patch] — done 2026-08-11

- Owner: `repos/mnemosyne/fuzz/Cargo.toml` (one version pin) plus the
  cascade audit record; no gitlink, lockfile, or provider manifest changes.
- The provider cascade landed in the worktrees (leto 0.42.0 `d9e674f`,
  moirai 0.5.0 `f68045d`, apollo-fft 0.26.0 `3373362`, hephaestus-wgpu
  0.19.0 `a68e91f`), all with gitlink == worktree head; the 202-line
  coherence drift collapsed to 3 consumer-side pins (helios ×2, mnemosyne ×1).
- Helios is peer-in-flight on `cascade/provider-042` (staged `Cargo.toml`
  moirai/hephaestus/gaia bumps) — not ours to touch; those 2 defects already
  left the live gate.
- Delivery (2026-08-11): committed `0568325` on
  `codex/mnemosyne-fuzz-cascade` bumping `mnemosyne-c-shim` 0.2.0 → 0.3.0 in
  `fuzz/Cargo.toml` (crate re-released to 0.3.0 in the mnemosyne 0.7
  re-release; fuzz is workspace-excluded, so the pin is standalone). Pushed;
  awaiting owner PR merge.
- Gate: `version-guard coherence` clean (235 manifests / 215 packages / 1036
  first-party requirements, 0 defects); fuzz `cargo metadata` re-parses
  with the `^0.3.0` path dep.

## ATLAS-BOOK-ANCHOR-PARITY-001 — Heading-id parity with mdBook v0.5.4 [patch] — done 2026-08-11

- Owner: `scripts/check_mdbook_links.py` plus one kwavers book chapter; no
  other book source, manifest, or gitlink changes.
- Root-causes the em-dash anchor divergence that SWEEP-001 band-aided with
  explicit `{#id}` anchors: the detector's heading slugs now match mdBook
  v0.5.4's render-then-normalize algorithm exactly (smart punctuation,
  per-whitespace-char hyphens, keep `-`/`_`/Unicode-alnum, no trim,
  `-1`/`-2` dedup, explicit `{#id}` verbatim and dedup-exempt, fenced-code
  masking).
- Verified two ways: (1) a 58-case empirical battery against the mdBook
  v0.5.4 binary (58/58), (2) full-book cross-validation — 346/346 chapter
  pages across all 24 provider books reproduce the built HTML heading ids
  byte-for-byte. The detector gate stays green (0/0/0); no book anchor
  links needed updating.
- Surface fix: kwavers `transcranial_ust_brain_imaging.md` — two `$$` math
  blocks whose bare `=` line was parsed as a CommonMark setext heading
  (garbage `-a_`-style ids in the built book); `=` joined onto the
  preceding line restores correct display math.

## ATLAS-BOOK-LINK-CI-001 — All-provider book link CI gate [patch] — done 2026-08-11

- Owner: root CI workflow, pre-commit hook, and parity documentation only;
  no provider source, manifest, or gitlink changes.
- Extend the existing `docs.yml` docs gate (previously only 5 physics books)
  to the full stack: strict `check_mdbook_links.py` + `mdbook build` across all
  `repos/*/docs/book` glob hits. All 24 books pass both today, so the gate is
  green at wiring time and self-maintaining for newly added books.
- Trigger paths cover non-`.md` book assets (`repos/**/docs/book/**`); the
  build loop surfaces the failing book; the strict gate is explicitly
  exit-code-based. Provider-side book edits surface at root CI as gitlink
  changes (no `paths:` match), so the gate is enforced at the root-commit
  boundary per the Atlas delivery model.
- Mirror the gate locally in `.githooks/pre-commit` and document the wiring in
  `MDBOOK_DETECTOR_PARITY.md` so the detector/mdBook parity contract stays
  honest.

## ATLAS-BOOK-LINK-SWEEP-001 — All-provider book link sweep [patch] — done 2026-08-10

- Owner: book documentation only; six ritk chapter files changed, no manifest,
  consumer, or gitlink changes.
- Run the Atlas `check_mdbook_links.py` detector across all 24 `repos/*/docs/book`
  trees. Only `ritk` failed: six `FILE_MISSING` (a nonexistent ritk-local
  `docs/adr/0036-neuroimaging-and-mr-ownership.md` cited by five chapters,
  `gaia_polyline` twice) and one `ANCHOR_MISSING` (the CSD section link).
- Fix the ADR citations to the canonical GitHub URL already used by
  `brain_tractography.md`; fix the CSD anchor with an explicit `{#id}` heading
  attribute because the Atlas detector's slug for the em-dash heading differs
  from mdBook's auto-slug, and apply the same `{#id}` hardening to the DTI,
  DKI, ODF, and NODDI model headings so the divergence class cannot recur.
- Evidence: the sweep now reports `FILE_MISSING: 0`, `ANCHOR_MISSING: 0`,
  `READ_FAIL: 0` for all 24 books; `mdbook build` passes for ritk with the
  explicit anchor; `git diff --check` is clean in both the ritk checkout and
  the Atlas root.

## ATLAS-TYCHE-PROVIDER-ESTIMATORS-001 — Tyche sensitivity estimators and book closure [patch] — done 2026-08-10

- Owner: Tyche provider source, tests, docs, and changelog; no consumer or
  release files are in scope.
- Deliver the two estimator increments previously deferred by
  ATLAS-PROVIDER-INTEGRATION-AUDIT-001: Morris elementary-effect screening
  (`ElementaryEffects`, `MorrisScreening`, `MorrisReport`) with a validated
  one-perturbation-per-parameter trajectory contract, and the Saltelli
  A/B/`A_i^B` first-/total-order estimator (`SobolIndices`, `SobolReport`)
  with explicit formulas, unit-interval clamping, and a two-row minimum.
- Close all five Tyche `Chapter prose deferred` book chapters with
  API-accurate prose covering moments, parameter spaces, the Sobol kernel,
  sensitivity screening, and the Atlas ownership split.
- Evidence: `cargo fmt`, `check --all-features` / `--no-default-features`,
  strict Clippy, full Nextest (including the new statistics_laws oracle and
  Monte Carlo Saltelli tests), doctests, warning-denied Rustdoc, `cargo
  deny`, mdBook build, placeholder scan, and `git diff --check` in a
  temporary clone (the checkout's peer-dirty `backlog.md`, `gap_audit.md`,
  `tests/bootstrap.rs`, and `Cargo.lock` were left untouched).
- Remaining: TYCHE-005 versioned Consus study schema, the score-only
  ensemble model, and crates.io release automation stay open and are
  recorded in `checklist.md` as such; `tyche-core` is already `publish =
  true` and its tag-gated crates.io publication is the open external release
  gate, while the facade and adapter crates stay `publish = false`.

## ATLAS-MNEMOSYNE-BOOK-001 — Complete Mnemosyne book closure [patch] — done 2026-08-10

- Owner: Mnemosyne provider documentation and changelog; no consumer or
  release files are in scope.
- Replace the ten `Chapter prose deferred — DoR item` book placeholders with
  API-accurate prose for size classes, allocation policies, the global
  allocator, scratch pools, the segment lifecycle, decay/purge/reset, NUMA
  placement, hardened/secure policies, profiling/leak detection, and the
  Atlas stack position, cross-referencing the two existing compiled example
  pages.
- Evidence target: `mdbook build`, the Atlas `check_mdbook_links.py` book
  link checker (zero broken links), and `git diff --check` in both the
  provider checkout and the Atlas root; the provider's peer-dirty
  `Cargo.lock`, `backlog.md`, and `gap_audit.md` remain untouched.
- Delivery (2026-08-11): committed `c4516df` on
  ...
  `9a143ca`), pushed; mdBook build and the portable link detector stay
  clean (0/0/0), `git diff --check` passes, no source or manifest change
  (the `Cargo.lock` overlay churn was excluded from the commit).
- Evidence: `version-guard scan` on `7541069..c4516df` clean (0 defects,
  rc=0) — the docs-only delivery carries no version movement. Root gitlink
  stays at merged `9a143ca` until owner PR merge.

## ATLAS-HYPERION-PROVIDER-DOCS-001 — Complete Hyperion book closure [patch] — done 2026-08-10

- Owner: Hyperion provider documentation; no consumer or release files are in
  scope.
- Replace the three deferred book chapters with API-accurate guidance for
  chromophore extinction spectra, Beer-Lambert optical depth/transmission and
  diffusion-derived coefficients, and the Atlas ownership split.
- Keep teaching snippets `rust,ignore` because mdBook's standalone harness has
  no Hyperion dependency manifest; CI's compiled `cargo test --doc` and the
  `photon_transport` / `chromophore_spectrum` / `book_mass_attenuation`
  examples remain the verification surfaces.
- Evidence target: provider formatting, no-default/all-feature checks, strict
  Clippy, Nextest, doctests, Rustdoc, cargo-deny, all three examples,
  placeholder scan, mdBook build, and `git diff --check`.
- Follow-up slice: add the compiled `book_mass_attenuation` example for the
  `MassAttenuation::to_linear` seam (Proteus density in, `LinearAttenuation`
  out), its `docs/book/examples/mass_attenuation.md` page, and the CI run
  step; no manifest, consumer, or gitlink change is part of either slice.
- Second follow-up slice: add the compiled `book_diffusion_deposition`
  example for the remaining diffusion/deposition seams — `reduced_scattering`
  and every `DiffusionCoefficients` derived law plus
  `absorbed_power_density`/`absorbed_energy_density` — with its
...
- Note: the checkout's pre-existing unstaged `Cargo.lock` edit (never staged,
  hence unrecoverable) was restored to the committed HEAD blob; the owner must
  re-apply any intentional lock change. No lockfile change is delivered.

## ATLAS-PROTEUS-PROVIDER-DOCS-001 — Complete Proteus book closure [patch] — done 2026-08-10

- Owner: Proteus provider documentation; no consumer or release files are in
  scope.
- Replace the three deferred book chapters with API-accurate guidance for
  validated material properties, constitutive-law evaluation, and the Atlas
  ownership split.
- Keep teaching snippets `rust,ignore` because mdBook's standalone harness has
  no Proteus dependency manifest; CI's compiled `cargo test --doc` and the
  `constant_material` / `temperature_material` examples remain the
  verification surfaces.
- Evidence: clean temporary-clone formatting, no-default/all-feature checks,
  strict Clippy, 18/18 Nextest, one doctest, Rustdoc, cargo-deny, both
  examples, placeholder scan, mdBook build, and `git diff --check` pass.
- Explicit residual: `proteus` remains `publish = false` (occupied registry
  name); registry release and exact-head gitlink delivery remain separate
  dependency-ordered work and are not reopened Phase 1 tasks. The checkout's
  pre-existing owner-review `Cargo.lock` edit was preserved untouched.

## ATLAS-HORAE-PROVIDER-DOCS-001 — Complete Horae book closure [patch] — done 2026-08-10

- Owner: Horae provider documentation; no consumer or release files are in
  scope.
- Replace the six deferred book chapters with API-accurate guidance for
  `ExplicitSystem`, const-generic tableaus, adaptive observations, borrowed
  event schedules, subcycle plans, and Atlas ownership boundaries.
- Keep teaching snippets `rust,ignore` because mdBook's standalone harness has
  no Horae dependency manifest; CI's compiled `cargo test --doc` and
  `cargo run --example ordered_decay` remain the verification surfaces.
- Evidence: temporary-clone formatting, no-default check, strict Clippy,
  15/15 Nextest, one doctest, Rustdoc, cargo-deny, ordered-decay example,
  placeholder scan, mdBook build, and `git diff --check` pass after Cargo
  reconciles the temporary lock graph. `mdbook test` remains blocked by the
  existing dependency-free harness for the included example, and committed
  locked Cargo validation remains a lock-graph residual; neither is claimed
  green here.
- Delivery (2026-08-11): closure prose committed `03ad868` on
  `codex/horae-book-prose` (based directly on `origin/main` `08cf292`) and
  pushed; mdBook build + portable link detector clean. Root gitlink stays at
  merged `08cf292` until owner PR merge.
- Explicit residual: `horae` remains `publish = false`; facade/publication,
  registry, consumer lockfile, and exact-head gitlink delivery remain separate
  dependency-ordered release work.

## ATLAS-AEQUITAS-PROVIDER-DOCS-001 — Complete Aequitas book closure — done 2026-08-10

- Owner: Aequitas provider documentation; no consumer or release files are in
  scope.
- Delivery: committed `11565d9` on `codex/aequitas-book-prose` (from
  `681042b`) and pushed 2026-08-11; mdBook build and portable link detector
  clean. Root gitlink stays at merged `681042b` until owner PR merge.

The eight Aequitas book chapters that were explicit `Chapter prose deferred`
placeholders are now complete: quantity model, canonical storage, dimension
algebra, linear units, scaled units, additive operations, derived quantities,
and Atlas stack position. The prose uses the committed `Quantity`,
`LinearUnit`, `MultiplyDimension`, and SI alias APIs and preserves the
Eunomia scalar/complex ownership boundary. The two included Rust source
listings are marked `rust,ignore` for mdBook's standalone harness; the
compiled examples remain provider-gated.

Clean-provider evidence at Aequitas `681042b`: locked metadata, fmt,
the CI no-default check, strict all-target/all-feature Clippy, Nextest 54/54,
13 doctests (including one compile-fail contract), Rustdoc, cargo-deny,
package listing, mdBook test/build, placeholder scan, and diff check pass.
The generated `Cargo.lock` was restored to the committed provider blob after
validation. Atlas-wide pin coherence and dependency-ordered facade/publication
work remain separate consumer/release gates.
...
## ATLAS-PROVIDER-INTEGRATION-AUDIT-001 — Audit nineteen Atlas providers [patch] — done 2026-08-11

- **Scope**: Horae, Hyperion, Themis, Tyche (aka Tychee), Proteus, Mnemosyne, Consus,
  Helios, Aequitas, Asclepius, Eunomia, Moirai, RITK, Melinoe, Leto,
  Hephaestus, Coeus, Apollo, and Iris.
- **Live state (2026-08-11)**: Atlas `b72d9f1` equals `origin/main`; the root
  gitlink remains the delivery SSOT. Fourteen requested child checkouts differ
  from their root pointers and five are aligned; Hermes is the sixth aligned
  checkout in the wider 25-member stack. All requested child worktrees are
  dirty and preserved.
- **Delivered in review**: Consus PR #16 and Moirai PR #118 carry verified
  provider-first integration increments. Their root gitlinks remain unchanged
  until exact merged hosted heads are green.
- **Acceptance**: every named repository has a current manifest/source/consumer
  audit, a classified residual or delivered slice, exact local evidence, and a
  dependency-ordered merge/gitlink path. Publication, hardware validation,
  owner review, and external integration remain separate gates.
- **Closure note (2026-08-11)**: Atlas integration-scope development/audit
  for the nineteen requested providers is complete. Remaining publication,
  hardware, and exact-head gitlink delivery work is explicitly tracked as
  external provider/release follow-up and is not reopened as an Atlas
  integration-identity gap.
- **Hardening increment (2026-08-11)**: closure enforcement is now automated
  in root tooling: `scripts/atlas-provider-integration-audit.py` checks
  requested-provider activation and root closure markers, and
  `scripts/atlas-version-guard-sweep.py` now composes toolchain preflight,
  global coherence, and this closure guard in CI.
- **Scoped coherence extension (2026-08-11)**: the provider-integration guard
  now reads `version-guard coherence --format json` and fails only on defects
  whose manifest path is inside the requested nineteen-provider scope; outside
  findings are reported as informational. Pre-commit runs the guard in
  `--structural-only` mode to avoid blocking local PM edits on unrelated
  version drift while still preventing marker/activation regressions.
- **Live scope snapshot (2026-08-11 local tree, refreshed)**:
  `version-guard coherence --format json` reports 0 global defects and
  `atlas-provider-integration-audit.py` reports the requested-provider scope
  clean. No in-scope version-skew defects are present in the current local
  integration state.

### Melinoe generativity continuation — 2026-08-10

- Melinoe remains the generativity SSOT: invariant higher-ranked brands,
  permit-gated cells, `WriterShard` disjointness, thread-local/sync token
  families, and conditional atomics are not duplicated in consumers.
- Added the alloc-gated `collections::with_generated` boundary and
  `BrandedVec::from_fn`. `with_generated` mints a fresh higher-ranked brand,
  generates indexed storage, and passes the branded vector plus its exclusive
  token to a callback whose return value may escape while the brand cannot.
  The API composes with the existing `partition_for_each_mut_with` phase rather
  than introducing a second scheduler or ownership model.
- Provider delivery is committed and pushed as Melinoe `47863b1`
  (`47863b12aa0cd4e65cb9556b2c9bbf1353a5ee26`, `feat(melinoe): Add generic
  branded generation`), on top of baseline `d272934`. Standalone locked
  evidence passes: branded-vector Nextest 15/15 with all features, 10/10 with
  `--no-default-features --features alloc`, 31/31 doctests, locked metadata,
  and rustfmt. The checked-out provider branch is currently at `709f28f` and
  carries unrelated peer edits in `CHANGELOG.md`, `Cargo.toml`, and
  `src/sync/scoped/partition/plan.rs`; those files were not committed or
  reset, and the Atlas gitlink remains at `d272934` pending owner-authorized
  pointer delivery. Apollo and Moirai consumer lockfiles now pin the delivered
  Melinoe revision `47863b12aa0cd4e65cb9556b2c9bbf1353a5ee26`.
- Themis, Gaia, and CFDrs lockfiles now resolve the committed Melinoe source
  at the full `47863b12aa0cd4e65cb9556b2c9bbf1353a5ee26` revision. Standalone
  locked gates pass for Themis (strict test compilation and branded Nextest
  17/17), Gaia (strict library check and permission-arena test), and CFDrs
  (strict `cfd-core` library check and Nextest 250/250). Themis's lockfile was
  already clean; Gaia and CFDrs retain broader pre-existing lockfile churn
  required by their peer manifest edits, now reconciled so `--locked` passes.
  The root overlay was temporarily bypassed for these checks and restored
  byte-for-byte; no overlay backup remains.
- Consumer adoption delivered in CFDrs
  `crates/cfd-core/src/physics/fluid_dynamics/operations.rs`: vorticity and
  divergence now use `melinoe::collections::with_generated`, expose the
  generated `BrandedVec` cells to the existing Moirai
  `par_partition_for_each` driver, and return through `into_vec()`. This
  removes the former unsafe `Vec<T>`→`MelinoeCell<T>` casts while retaining
  consumer-owned finite-difference formulas and Moirai scheduling.
- Consumer evidence: CFDrs offline metadata and `cargo check -p cfd-core
  --lib` pass; focused `cargo nextest run -p cfd-core` passes 269/269; source
  formatting passes. CFDrs's active peer manifest removal of `num_cpus` leaves
  its existing Cargo.lock stale for `--locked`, and strict Clippy remains
  blocked by the unrelated pre-existing `compute/backend.rs:53`
  `clippy::map_unwrap_or` lint. Those peer-state blockers are not claimed as
  adoption failures.
- Direct Melinoe source seams are present in Gaia, Themis, Mnemosyne, Moirai,
  CFDrs, and Apollo; RITK/Helios/Kwavers feature plumbing remains intentionally
  indirect where Moirai or Themis owns the boundary. Gaia's permissioned arena
  now stores values in Melinoe `BrandedVec`, exposes zero-copy token-gated slice
  views, and provides a fresh-brand `permission::with_generated` constructor;
  its focused regression target is 3/3 and the full Gaia suite is 969/969
  (one skipped). Gaia's pre-existing `Cargo.lock` contains only `[[patch.unused]]`
  churn and remains outside this delivery slice.  Themis's dynamic and const-generic `NumaPinnedSlice` constructors now
  consume Melinoe's `BrandedVec::into_boxed_cells` handoff, including indexed
  `from_fn` generation, preserving Themis-owned placement identity while
  eliminating duplicated `Vec<T>`→`Vec<MelinoeCell<T>>` construction. Themis
  placement permits now expose placement-gated parallel mutation for uniquely
  owned pinned slices and delegate shard execution to Melinoe's existing
  partition driver; borrowed reference slices remain read/write token paths
  and do not gain an ownership bypass. Themis strict feature-matrix checks and
  branded tests pass 17/17 offline; its existing lockfile drift remains
  peer-owned. Themis's remaining `Cargo.toml`, `README.md`, and
`src/topology/cpu/{cache.rs,mod.rs}` edits are pre-existing peer dirt and are
excluded from this migration; no other peer-owned consumer files were modified.
...
- **Tyche** (`repos/tyche` HEAD `d25311e`) owns reproducible study identity,
  ...
  provider/consumer scope: Morris and Saltelli estimators are delivered by
  `ATLAS-TYCHE-PROVIDER-ESTIMATORS-001`; complete the versioned Consus study
  schema, address the score-only ensemble model before routing it through
  provider resampling, and finish crates.io release  automation. Tyche's `backlog.md` and `gap_audit.md` were already dirty peer-
  owned files; the audit-001 continuation itself changed no Tyche files and
...
- **Historical worktree snapshot (2026-08-09):** the earlier ownership
  classification is retained as provenance only; it is superseded by the live
  post-fetch reconciliation below. Do not use its clean/dirty or continuation
  labels as current state.
- **Live checkout reconciliation (2026-08-11 after `git fetch --prune`; Atlas
  `b72d9f1`, `main` equals `origin/main`):** the recorded Atlas gitlink is the
  delivery revision. The current child checkout equals that revision only for
  Themis `038457d`, Tyche `d25311e`, Aequitas `681042b`, Mnemosyne `9a143ca`,
  and Coeus `4491bf1`; the remaining requested child checkouts are branch or
  stale-ancestor checkouts and must not be mistaken for delivery candidates.
  Current checkout → recorded gitlink pairs are Horae `8877388` → `08cf292`,
  Hyperion `f84e27a` → `9a8b7d8`, Proteus `2918e5a` → `3d6021e`, Consus
  `c2064d4` → `ae47946`, Helios `34f3cf2` → `3494eda`, Asclepius `2e4b410`
  → `530115a`, Eunomia `b9801fc` → `184ba92`, Moirai `ecc6020` → `905b5f5`,
  RITK `10f0d0a` → `59baeba`, Melinoe `deae202` → `eab19a6`, Leto `1c36ce1`
  → `ca93b63`, Hephaestus `4c0fd57` → `d4d5906`, Apollo `8202859` →
  `0e38d1c`, and Iris `9528748` → `ab3eea2`. The requested checkout dirt is
  material in Horae (6 files), Hyperion (9), Themis (8), Tyche (14), Proteus
  (4), Mnemosyne (14), Consus (4), Helios (3), Aequitas (10), Asclepius (1),
  Eunomia (4), Moirai (16), RITK (53), Melinoe (5), Leto (6), Hephaestus (3),
  Coeus (2), Apollo (8), and Iris (1). No child checkout is reset, stashed,
  committed, or advanced by this reconciliation; dirty scopes require their
  own ownership and gate decisions.
- **Delivery rule:** do not advance a gitlink from a non-main or dirty child
  checkout. A provider delivery increment requires a committed provider head,
  exact locked gates, provenance against fetched `origin/main`, and a separate
  Atlas gitlink commit. The old 2026-08-10 ledger is superseded by this live
  snapshot.
...
## ATLAS-IRIS-CLOSURE-001 — Iris IRIS-003 release-readiness record [chore] — done 2026-08-11

- Owner: current session. Scope: deliver the provider-local IRIS-003
  release-readiness record authored in the iris worktree (backlog/checklist)
  as a clean delivery commit on `codex/iris-closure-record` at origin/main
  `ab3eea2`, pushed. The record states the `iris` 0.1.0 name is unregistered,
  the OIDC publishing automation is merged, and the local gates pass;
  `cargo package --locked` remains overlay-blocked and hosted publish steps
  stay external.
- Evidence: 2026-08-11 local re-verification passes fmt, all-feature checks,
  warning-denied Clippy, 16/16 Nextest, 2/2 doctests; `version-guard scan`
  clean (0 defects), stack-wide coherence clean (0 defects); the 188-line
  `Cargo.lock` diff was ambient-overlay re-resolution (eunomia/horae/moirai →
  mnemosyne/hyperion/themis/consus patch set + `syn` 3.0.2→3.0.3) and was
  restored to the committed blob, not delivered. Root gitlink stays at merged
  `ab3eea2` until owner PR merge.

## ATLAS-ASCLEPIUS-BOOK-001 — Complete Asclepius book closure [patch] — done 2026-08-11

- Owner: current session. Scope: replace all four `Chapter prose deferred`
  book placeholders in asclepius (`response_values`, `geud`, `tcp_ntcp`,
  `stack_position`) with API-accurate prose and deliver as
  `codex/asclepius-book-closure` at origin/main `530115a`, pushed.
- Evidence: mdBook build + portable link detector clean (0/0/0);
  `version-guard scan` clean (0 defects), stack-wide coherence clean
  (0 defects); docs-only delivery touches no `Cargo.toml`. 2026-08-11
  re-verification of the three compiled examples under the overlay
  (`treatment_response` gEUD=50.662 Gy/TCP=0.52629/CEM43=3.000 min,
  `book_geud` uniform-mean-max anchor assertions, `book_biological_values`
  full validation surface) passes rc=0; overlay `Cargo.lock` churn restored
  after the runs. Root gitlink stays at merged `530115a` until owner PR
  merge; crates.io publish + trusted publisher remain external release
  gates.

## ATLAS-COEUS-MLM-PROVIDER-001 — Coeus multi_label_margin_loss provider delivery [patch] — done 2026-08-11

- Owner: current session (branch push only; commit authored by the provider
  owner). Scope: push `feat/mlm-provider` at `1ac8118c` so the delivery is
  remotely visible for owner PR review; the root gitlink already records this
  head. The commit migrates `multi_label_margin_loss` to provider ownership
  (pairwise [N,C,C] active-tensor broadcast, safe flattened target gather,
  masked positive hinge, target/sibling scatter backward, 4 value-semantic
  tests) and closes the last non-sequential host-staged loss family; CTC
  remains the sole sequential-DP exception.
- Evidence: `version-guard scan` on `4491bf19..1ac8118c` clean (0 defects);
  stack-wide coherence clean at scan time (live rerun: 24 hermes-drift
  defects, see ATLAS-CGROUP-CLOSURES-001); the in-range `Cargo.lock` change
  is the ambient patch-set re-resolution signature with no version movement.
  Compile/test re-verification (2026-08-11): `cargo build -p coeus-autograd
  --all-targets` rc=0 and `cargo test -p coeus-autograd multi_label_margin`
  passes 4/4 (forward reference, backward analytic, two-valid-targets,
  should-panic target-length reject) under the Atlas overlay. Extended gate
  (2026-08-11): full `cargo test -p coeus-nn` 322 passed / 2 ignored / 0
  failed; full `cargo test -p coeus-autograd` 178 passed / 0 failed; strict
  `cargo clippy -p coeus-nn --all-targets -- -D warnings` and `cargo clippy
  -p coeus-autograd --all-targets -- -D warnings` both clean (rc=0). The
  full-suite runs needed the overlay's `hermes-simd` patch pointed at a
  temporary gitlink checkout (`bde7010f`, hermes-simd 0.5.0, under
  `target/hermes-gitlink`) via `cargo --config` because the hermes
  worktree's eunomia-0.8 re-release provides 0.6.0; no peer-owned hermes  dirt was touched and the temp checkout was removed. Overlay `Cargo.lock`
  churn restored, worktree clean at `1ac8118c`. Post-gate version-graph
  re-verification (2026-08-11): coeus `Cargo.lock` byte-identical to the
  committed blob; per-commit `version-guard scan` on `4491bf19..1ac8118c`
  still 0 defects (rc=0); stack-wide coherence unchanged (235/215/1048, 24
  hermes-drift defects with the same composition — none introduced by the
  gate runs). Root gitlink stays at `1ac8118c` until owner PR merge.

## ATLAS-HELIOS-DICOM-ORIENTATION-001 — Helios DICOM oriented-grid boundary delivery [patch] — done 2026-08-11

- Owner: current session. Scope: commit the previously worktree-only Helios
  DICOM orientation work and push `codex/helios-dicom-orientation` at
  `6858282` (based directly on `origin/main` `342bbbc83` = the root gitlink).
  The commit enforces `ImageOrientationPatient` in `load_ct_slice` /
  `load_ct_series` (oriented `VoxelGrid` construction, slice-normal-projected
  series stacking, dot/cross/normalize helpers with an orientation tolerance,
  synthetic-provider orientation-pose tests through `ritk-dicom`) and syncs
  CHANGELOG.
- Evidence: `cargo check -p helios-domain --features dicom --all-targets`
  rc=0; `cargo test -p helios-domain --features dicom` 44/44; strict clippy
  `-- -D warnings` clean (rc=0); fmt + `git diff --check` clean; no
  hermes-simd resolution blocker at the crate level; overlay `Cargo.lock`
  churn restored. Full-workspace re-verification (2026-08-11, pre-owner-PR):
  `cargo check --workspace --all-targets` compiles all 11 crates clean
  (rc=0, 52.41s, zero errors) with the same temp-gitlink hermes bypass as
  the coeus gate — the workspace pulls `hermes-simd` transitively via
  `helios-gpu → hephaestus-wgpu → leto-ops v0.41.0` (requires `^0.5.0`,
  overlay patch provides the hermes worktree's 0.6.0 at `77716bb`), so the
  gate ran against a temporary hermes checkout of the gitlink `bde7010f`
  (hermes-simd 0.5.0) under `target/hermes-gitlink` via `cargo --config`;
  temp removed, peer hermes dirt untouched, lock churn restored.
  `version-guard scan` on `342bbbc83..6858282` clean (0 defects, rc=0) — no
  `Cargo.toml` touched; stack-wide coherence unchanged. Root gitlink stays at
  merged `342bbbc83` until owner PR merge; the stale `codex/helios-lock-fix`
  branch (3 commits already merged into main) and the peer ADR-index
  `docs/adr/README.md` edit stay untouched.

## ATLAS-LETO-HERMES-REDUCED-PRECISION-001 — Leto F16/Bf16 Hermes provider delivery [patch] — done 2026-08-11

- Owner: current session. Scope: commit the previously worktree-only Leto
  reduced-precision provider work and push `codex/leto-hermes-reduced-`
  `precision` at `606e5b5` (based directly on `origin/main` `d9e674fc`, which
  already carries the eunomia-0.8 cascade re-release incl. leto 0.41.0 and
  `hermes-simd` 0.6.0). The commit routes `leto-ops::SimdStrategy` F16/Bf16
  elementwise, reduction, AXPY, GEMV, and GEMM operations through the same
  capability-checked Hermes provider as `f32`/`f64`, replacing the scalar
  unsupported stubs; adds F16 elementwise/sum/dot provider tests and the
  AXPY-accumulation test; syncs CHANGELOG/backlog/checklist/gap_audit.
- Evidence: `cargo check -p leto-ops --all-targets` rc=0; `cargo test -p
  leto-ops` 544 passed / 1 ignored / 0 failed (incl. the two F16/Bf16
  provider tests and `norms_run_at_reduced_precision`); strict clippy `-D
  warnings` clean (rc=0); fmt + `git diff --check` clean; overlay
  `Cargo.lock` churn restored. `version-guard scan` on `d9e674fc..606e5b5`
  clean (0 defects, rc=0) — no `Cargo.toml` in range. Worktree restored to
  the root gitlink `ca93b63c` after the push (the origin-main-based checkout
  transiently reported 122 coherence defects — leto 0.41.0 vs consumers at
  0.40.0 — which the restore resolved; live gate is now 1 residual line,
  `leto-ops`@gitlink `hermes-simd` 0.5.0 vs 0.6.0, closing when leto's
  re-release lands). Root gitlink stays at merged `ca93b63c` until owner PR
  merge; the stale `codex/leto-git-lock` branch (redundant `hermes-simd`
  0.6.0 bump `d68095b`) and the tool-generated ADR-index edit stay
  untouched.

## ATLAS-APOLLO-SHARED-VALIDATION-001 — Apollo shared WGPU transform validation delivery [patch] — done 2026-08-11

- Owner: current session. Scope: deliver the worktree-only Apollo
  D8-shared-validation extraction as `b426f2cd` on
  `codex/apollo-shared-validation` (based directly on `origin/main`
  `0e38d1cc` = the root gitlink), pushed 2026-08-11. `WgpuTransformBackend`
  exposes canonical non-empty-plan, operand-length, and typed-storage-profile
  validators; `apollo-gft` consumes them and retains only graph-basis shape
  validation; direct tests pin the shared invalid-plan, length-mismatch,
  profile-match, and profile-mismatch behavior; the validation suite gains a
  mnemosyne branded-slice boundary integration test (additive
  `apollo-validation` dev-dependency on the workspace `mnemosyne`);
  CHANGELOG/backlog/gap_audit synced.
- Evidence: `cargo check --workspace --all-targets` rc=0 (all 20 crates);
  `cargo test -p apollo-fft shared_ --features wgpu` 3/3, `-p apollo-gft
  basis` 2/2, `-p apollo-validation mnemosyne_branded` 1/1; strict clippy
  `--all-targets -- -D warnings` clean on apollo-fft (wgpu), apollo-gft, and
  apollo-validation; fmt + `git diff --check` clean. The origin/main base
  requires `hermes-simd` 0.5.0 while the overlay patches to the local hermes
  worktree's 0.6.0, so gates ran with the temp-gitlink bypass (hermes
  `bde7010f` under `target/hermes-gitlink` via `cargo --config`); temp
  removed, peer hermes dirt untouched, overlay `Cargo.lock` churn restored.
  `version-guard scan` on `0e38d1cc..b426f2cd` clean (0 defects, rc=0) — no
  version movement, additive dev-dependency only; stack-wide coherence
  unchanged (apollo contributes 0 defects; the live gate holds 2 residual
  lines from the pre-existing hermes/leto drift, both non-apollo). Root
  gitlink stays at merged `0e38d1cc` until owner PR merge.
- Collision note: the owner's live automation advanced `build/hermes-simd-0.6`
  (`eae6b706`, hermes-simd 0.6.0) in this worktree between branch creation
  and commit, so commit `1931da70` (identical content) landed on that peer
  branch and the initial push carried only the base. The canonical delivery
  was rebuilt by cherry-picking onto the gitlink base in a temporary worktree
...
- Owner: current session. Scope: commit the previously worktree-only Gaia
  permissioned-arena adoption and push `codex/gaia-permissioned-arena` at
  `b5e62c5` (based directly on `origin/main` `5ea09cbc`). The commit migrates
  `PermissionedArena` storage to `melinoe::collections::BrandedVec` behind
  the existing `GhostToken` facade, adds zero-copy token-gated
  `as_slice`/`as_mut_slice` views, a fresh-brand
  `permission::with_generated` scoped constructor, and a new
  `tests/permission_arena.rs` with 3 integration tests.
- Evidence: `cargo check --all-targets` rc=0 and the three `permission_arena`
  integration tests pass 3/3 under the Atlas overlay (2026-08-11); overlay
  `Cargo.lock` churn restored, worktree clean. `version-guard scan` on
  `5ea09cbc..b5e62c5` clean (0 defects, rc=0) — no `Cargo.toml` touched (the
  `melinoe = "0.9.0"` dependency is already committed at the base). Root
  gitlink stays at merged `a5b0fe72` until owner PR merge.

## ATLAS-CGROUP-CLOSURES-001 — C-group closure sweep (melinoe/moirai/proteus/consus) [chore] — done 2026-08-11

- melinoe — already closed by the owner: the book closure (chapters +
  book.toml edition fix) landed via merged PRs #10 (`eab19a6`) and #11
  (`c8e8889`); main `6d80c33` carries the prose, worktree clean.
- moirai — already closed by the owner: the concurrency-regression hardening
  (event-gated scheduling, value-checked joins) plus CHANGELOG/CHECKLIST
  records merged via PR #118 at the root gitlink `57c4ec4` (commit
  `2bf516b`); worktree clean.
- proteus — clean at the gitlink (`0003266` = origin/main); the former
  `Cargo.lock` overlay churn is gone. No work remaining.
- consus — restored the comment-only `Cargo.toml` reword (plus CRLF
  normalization) and the `Cargo.lock` overlay churn to the committed blob;
  the worktree is clean at `8b77949` (an ancestor of origin/main,
  byte-identical on Cargo files). No substantive change was lost.
- Residual (not C-group scope): after restoring the gaia/mnemosyne worktrees
  to their gitlink-aligned heads, `version-guard coherence` reports 24
  requirement defects from the hermes worktree having advanced past its
  gitlink with the eunomia-0.8 re-release (`hermes-simd` 0.6.0 /
  `mnemosyne-memory` 0.7.0) while apollo (×18), kwavers (×2), and
  CFDrs/coeus/hermes/leto (×1 each) still require the older versions; this
  arose independently of the closures and resolves with the hermes D-group
  delivery.

## ATLAS-VERSION-GUARD-SCAN-MATRIX-001 — per-commit scan matrix [chore] — done 2026-08-11

- Owner: current session. Scope: consolidated per-commit `version-guard scan`
  matrix for all ten delivery branches, mirrored from `gap_audit.md`. Every
  range is exactly the single delivery commit; all worktrees sit at their
  delivery heads; all ten report 0 version-bearing lines touched, rc=0,
  `{"defect_count":0,"findings":[]}`. The themis commit is the only
  `Cargo.toml`-touching one (`melinoe/alloc` feature wiring, no version
  movement); coeus is the only source delivery (its in-range `Cargo.lock`
  change is the ambient patch-set re-resolution signature). All ten branches
  are pushed and await owner PR merge.
- Evidence: ten live scan runs (2026-08-11), each 0-defect, plus the live
  stack-wide `coherence` companion — 235 manifests / 215 packages / 1048
  first-party requirements. The original 0-defect companion predates the
  hermes worktree advancing past its gitlink; the live rerun (2026-08-11)
  reports `DEFECT` with 24 requirement defects from that eunomia-0.8
  re-release drift (23× consumers require `hermes-simd` 0.5.0 vs actual
  0.6.0; 1× `hermes-simd-core` requires `mnemosyne-memory` 0.7.0 vs the
  gitlink-aligned 0.6.0). The per-commit rows above are unaffected and the
  drift closes with the hermes D-group delivery.
...
- Owner: Themis provider source + tests; the pinned melinoe rev `47863b1`
  (ancestor of `origin/main`) already provides every consumed API, so no
  consumer or melinoe change is required.
- Delivery: committed `cad222b` on `codex/themis-melinoe-adoption` (based
  directly on `origin/main` `038457d`) and pushed 2026-08-11. Scope:
  `BrandedVec` handoff for `NumaPinnedSlice`/`ConstNumaPinnedSlice`
  construction and `from_fn`, `partition_for_each_mut_with` on dynamic and
  const placement permits, no-`std` `alloc` fix, dead no-`std`
  `detect_cache_levels` fallback removal, `melinoe/alloc` feature wiring,
  README boundary note, and two new branded tests.
- Evidence: strict clippy `--all-targets`, Nextest 21/21 default, 38/38 with
  `testing`, 21/21 `--no-default-features`, mdBook build + link detector,
  `git diff --check`, no Cargo.lock churn. Compile ran under the Atlas
  overlay (local melinoe checkout); the pinned-rev API surface is
  source-verified, so committed `--locked` validation remains a lock-graph
  residual. Root gitlink stays at merged `038457d` until owner PR merge.
...
- Evidence (updated 2026-08-07 after Helios H-100 commit `234574c`):
  ...
  - **melinoe**: this is superseded by the later 2026-08-10 generativity
    continuation above. At this item's 2026-08-07 snapshot, the three named
    integrators had no direct Melinoe source imports beyond Moirai feature
    plumbing; current direct seams are recorded in the active audit (including
    CFDrs flow-field writes), while Helios and Kwavers remain intentionally
    indirect through Themis/Moirai.
...
## ATLAS-SUBSTRATE-001 — Extend the Hephaestus operation seams to Coeus's call surface [arch] [minor] — done 2026-08-11
...
- Acceptance met 2026-08-11 at gitlink `a68e91f`: the three new seams now have
  implementors on all four backends — `hephaestus-wgpu`
  `application/{elementwise,scan,axis_reduction,full_reduction}_seam.rs`,
  `hephaestus-cuda` the same set, `hephaestus-metal`
  `{elementwise,scan,axis_reduction,full_reduction}_seam.rs`, and
  `hephaestus-rocm` the same set; each backend carries the conformance
  contract-test binaries (5-6 files each: elementwise/typed_elementwise/scan/
  axis_reduction/full_reduction contracts). Verified under the overlay:
  `cargo check -p hephaestus-core -p hephaestus-conformance -p
  hephaestus-host --all-targets` rc=0; `cargo test -p hephaestus-core`
  89+1 passed; strict clippy `-D warnings` rc=0; `cargo check -p
  hephaestus-wgpu --tests` rc=0 (GPU execution remains the external hardware
  gate). The tyche overlay bypass is retired 2026-08-11: the peer's
  `cascade/moirai-0.5` advance `e245cf8` committed the root manifests
  (restoring `edition.workspace` inheritance); clippy and wgpu contract-test
  gates rerun clean with no `--config` patch. SUBSTRATE-002's provider half is
  landed
  (see that item) — the vendor deletion ledger remains.
...
- Owner: codex-2026-08-11; scope: `repos/coeus/crates/coeus-hephaestus` (one generic
  ...
  2026-08-11: `coeus-hephaestus` holds the generic elementwise/reduction
  providers (elementwise.rs 485 lines, reduction.rs 313 lines), referenced by
  all four vendor crates (6/8/8/6 files each); provider tests pass 6+1.
  Metal/rocm deletion slice delivered 2026-08-11: `2f3af87e` (+ doc
  follow-up `9167f574`) on `codex/coeus-provider-deletion-metal-rocm`
  (pushed) deleted all fourteen
  cloned metal/rocm backend modules (`{elementwise,runtime,reduction,
  cross_entropy,random_init,rotate_half,stateful_update}.rs`), replaced the
  vendor `MetalBackend`/`RocmBackend` with `HephaestusBackend<Provider>` plus
  provider op-bundle declarations, migrated the metal/rocm tests, added
  `random_init`/`rotate_half` `implementation.rs` to the bridge, and recorded
  the migration contract in ADR 0060. Gates: check rc=0 (all targets),
  provider tests 6+1, metal/rocm test binaries compile on host (runtime
  device-gated), strict clippy rc=0, fmt/diff clean, version-guard scan 0
  defects, stack coherence clean. Cuda deletion slice delivered 2026-08-11
  (branch `codex/coeus-provider-deletion-cuda`, pushed): `CudaBackend` keeps
  its type and declares the provider op bundles
  (`ElementwiseProvider<f32|i32>`, `ScalarPowerProvider<f32|f64>`,
  `ReductionProvider`) whose `coeus_ops` impls delegate through
  `HephaestusBackend<CudaBackend>` via the zero-copy
  `HephaestusStorage::from_arc` seam; the cloned NVRTC `math/elementwise/*`
  layer and `kernels/launch_ops/*` launchers are deleted (−273 net lines)
  with the public launch re-exports; rank rejection keeps the historical
  ...
  doctests pass, strict clippy rc=0, fmt/diff clean, stub path green.
  `f64` elementwise deferred (hephaestus-cuda comparison
  `TypedBinaryExpr<CudaC, f64>` gap, recorded in coeus `docs/gap_audit.md`).
  Wgpu deletion slice delivered 2026-08-11 (branch
  ...
  conversion and free-function dispatch helpers are deleted (301 → 84
  lines); the fused reduction path is unchanged;
  `From<HephaestusBackendError> for WgpuBackendError` preserves the
  historical `Validation(UnsupportedRank)` wire contract. Gates: check rc=0
  with 0 code warnings, strict clippy rc=0, fmt/diff clean, doc tests 5/5;
  the device-dependent tests fail only with the pre-existing
  `AdapterUnavailable` hardware gate. The SUBSTRATE-002 vendor deletion
  ledger is CLOSED; only the per-hardware physical-device contract-test
  execution remains (external hardware gate).
- CI-independence evidence (2026-08-11): the merged metal/rocm delivery was
  rebuilt against a temporary clean clone of hephaestus checked out at the
  exact committed gitlink `3be20f436aa2` (`target/hephaestus-gitlink`, dirty
  count 0), with both hephaestus `[patch]` sections overridden via
  `cargo --config`; `cargo metadata` confirmed `hephaestus-core`,
  `hephaestus-metal`, and `hephaestus-rocm` resolved exclusively from the
  temp clone manifest paths. Gates against the clean checkout: check
  (all-targets) rc=0 with 0 code warnings, tests 2+1+1 passed (device test
  binaries compile on host), strict clippy `-D warnings` rc=0. The temp
  clone was removed and the overlay restored byte-for-byte; the delivery has
  no dependency on the peer hephaestus worktree state, so a clean CI clone
  builds identically. Extended 2026-08-11 to the cuda and wgpu slices
  (stacked branches `codex/coeus-provider-deletion-cuda` `d7077547` and
  `codex/coeus-provider-deletion-wgpu` `c1380bce`, both in the checked-out
  history): the same `target/hephaestus-gitlink` clean checkout at
  `3be20f436aa2` was used with both hephaestus `[patch]` sections
  overridden via `cargo --config`, and `cargo metadata` confirmed
  `hephaestus-core`, `hephaestus-cuda`, and `hephaestus-wgpu` resolved
  exclusively from the temp clone manifest paths. Gates against the clean
  checkout (all three hephaestus `[patch]` crates redirected, incl. the
  `hephaestus-cuda/cuda` feature path): check cuda rc=0 (0 code warnings)
  and check wgpu rc=0 (0 code warnings); cuda no-device suite 25 lib + 99
  parity + 2 doctests pass; wgpu error-contract unit tests 7/7 and doc
  tests 5/5 pass (device suite remains the pre-existing `AdapterUnavailable`
  hardware gate); strict clippy `-D warnings` rc=0 with 0 real warnings
  (only the environmental unused-patch notices); fmt + `git diff --check`
  clean. Temp clone removed and overlay restored byte-for-byte; all three
  deliveries are proven independent of the peer hephaestus worktree state.
  Workspace-level extension (2026-08-11): the full coeus workspace
  (`cargo check --workspace --all-targets`, all 16 member crates) was built
  against the same clean `3be20f436aa2` checkout with all five hephaestus
  `[patch]` crates redirected; `cargo metadata` confirmed the whole
  workspace graph resolves `hephaestus-core/cuda/wgpu/metal/rocm`
  exclusively from the temp clone, rc=0 with 0 errors and 0 code warnings
  (the sole `unused variable: device` note is the known provider-owned
  `hephaestus-cuda` lint at the temp clone, not coeus code). Strict
  `cargo clippy --workspace --all-targets --no-deps -- -D warnings` against
  the clean checkout rc=0 with 0 real warnings. No hermes-simd bypass was
  needed: the hermes worktree is aligned at its gitlink `03e5e175` (only
...
- Acceptance: a normalized diff between `coeus-rocm` and `coeus-metal` shows no
  ...
  deleted.
- Claim update 2026-08-11: reclaiming the first deletion increment in the clean
  Coeus primary checkout; scope is `coeus-metal` and `coeus-rocm` cloned
  elementwise/reduction/runtime modules and their directly coupled tests. The
  CUDA and WGPU deletions remain separate follow-on increments.
