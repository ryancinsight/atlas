...
| ATLAS-KWAVERS-KZK-LINEAR-080 | **Live wrong physics on a therapy path.** `kwavers-solver/src/forward/nonlinear/kzk_solver_plugin/solver.rs::apply_linear_step` is reachable in production - called at `solver.rs:151,153,194,196` and `plugin_impl.rs:49,58`, registered in `plugin/catalog.rs` for `NonlinearEquation::KZK`, and driven from `kwavers-therapy`'s orchestrator (`therapy_integration/orchestrator/execution.rs`). Three independent errors, all verified at head: (1) **real-space and spectral indices are conflated** - `initialize_operators` builds `kx = TWO_PI * i / (nx * dx)` from the *spatial* loop index at `solver.rs:86`, and `apply_linear_step` then multiplies `field[[i, j, f_idx]]`, a real-space cell, by the operator at that same index; (2) the diffraction operator is applied as a **real** `cos(.)` multiplier where the parabolic propagator is `exp(-i(kx^2+ky^2)/(2k) dz)`, so it attenuates instead of rotating phase; (3) `absorption.powf(step_size)` raises a per-step factor to an exponent **in metres** - for `absorption = exp(-alpha*dz)` that yields `exp(-alpha*dz^2)`, dimensionally wrong. Found while removing the dead mocks of ATLAS-KWAVERS-REAL-COMPUTE-028; the dead code was the decoy and this is the live defect beside it. A correct KZK implementation already exists at `forward/nonlinear/kzk/`, so the likely resolution is to retire this plugin's hand-rolled operator path onto it rather than repair three errors in a duplicate. | [major] | The linear step operates in the domain its operators are defined in (FFT to spectral, apply, inverse) or delegates to `forward/nonlinear/kzk/`; the propagator is complex-valued; absorption over a step of `dz` is `exp(-alpha*dz)` with no metre-valued exponent. Verified against an analytical oracle, not self-consistency: on-axis focused-beam amplitude for a linear parabolic propagator has a closed form, and a plane wave through homogeneous absorption must decay as `exp(-alpha*z)` with the observed rate matching `alpha` to a derived tolerance. Each test fails on the parent commit. |
...
| ATLAS-COEUS-LINT-RATCHET-097 — **in progress 2026-08-17** | The previous floor closure is contradicted by the current recorded Coeus default `32d7f4e`: the clean hosted Atlas scan reports `coeus/allow_sites: 12 -> 90`, while the provider contains 95 `#[allow(...)]` occurrences across source and tests. The peer-dirty Coeus checkout is excluded; this item owns a clean lane from fetched `origin/main`. | [patch] | Reduce production `#[allow(...)]` to zero by fixing the lint or replacing each justified suppression with a scoped `#[expect(..., reason = "ratchet ATLAS-COEUS-LINT-RATCHET-097")]`; run provider format/check/test gates, regenerate the Atlas conformance baseline only after the count is reduced, and pass the exact-head hosted ratchet. |
...
### ATLAS-APOLLO-PRINT-098 — active provider cleanup [patch]

Apollo’s recorded default `f2aa537` leaves `apollo-bench/src/suite.rs:53`
writing a report from a library method even though `report()` already returns
the complete value. This is the clean hosted `apollo/print_dbg: 6 -> 9`
regression class. The item moves output ownership to the binary boundary by
deleting the unused library emitter.

Acceptance: Apollo source has zero production print/debug macros in the Atlas
scan; `apollo-bench` format/check/test and semver gates pass; the provider PR
merges at a green exact head; and Atlas conformance no longer reports the
Apollo print regression.
...
| ATLAS-MSRV-UNVERIFIED-077 | **Melinoe and Eunomia slices closed 2026-08-14.** Melinoe tracks a standalone lockfile and hosted Rust 1.65.0 all-target check (`31785253730` at `6e6a181`). Eunomia now has a hosted Rust 1.95.0 all-target/all-feature gate (`31789001841` at `b6c3d9a`), an exact online package dry-run at default `84c82fe`, and Atlas pointer integration at the current provider default. Mnemosyne remains the only open portion. | [patch] | Each declared floor has a hosted build, or the floor is raised to what the code requires |
