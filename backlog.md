...
| ATLAS-RITK-DICOM-ORIENTATION-070 | **In progress — claimed 2026-08-14.** Extend RITK’s provider-owned DICOM tag SSOT with `ImageOrientationPatient` (0020,0037), add a value-semantic provider test, and replace Helios’s local duplicate with the provider constant. Preserve the existing orientation normalization and grid behavior; do not touch Helios’s peer-owned typed-slope lockfile work or RITK’s `ritk-fix` lane. Acceptance: provider and consumer focused gates pass, no local duplicate remains, the exact provider head is merged, and Atlas advances both gitlinks only after the co-evolution gates pass. | [minor] | ATLAS-RITK-DICOM-ORIENTATION-070 |
