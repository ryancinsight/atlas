...
| ATLAS-MSRV-UNVERIFIED-077 | **Melinoe slice closed 2026-08-14.** Eunomia slice claimed by the current session. Melinoe now tracks a standalone lockfile and runs all-target `cargo check --locked --all-features` at declared Rust 1.65.0; hosted run `31785253730` passes at source head `6e6a181`. Mnemosyne remains open. Eunomia's local Rust 1.95 all-target/all-feature check passes; a provider-owned hosted gate is in progress. | [patch] | Each declared floor has a hosted build, or the floor is raised to what the code requires |

**Eunomia claim (2026-08-14):** scope is `repos/eunomia/.github/workflows/msrv.yml`
and the provider's E-REL-001/closure PM records. Acceptance is a hosted Rust 1.95
`cargo check --locked --workspace --all-targets --all-features` at the merged
provider default head, plus the online `cargo publish --locked --package eunomia
--dry-run` already passing locally. No source or lockfile change is authorized by
this claim unless the floor check falsifies the current manifest contract.
