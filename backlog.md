...
| ATLAS-MOIRAI-ORDERING-052-POOL | Audit and reduce the async `ConnectionPool::reserved_connections` update orderings. Acceptance: a Loom model covers serialized admission racing paired cancellation without underflow or capacity overrun; connection-pool and network stop tests remain value-correct; hosted provider gates pass. Scope: `moirai-async/src/net/types.rs` and its reservation protocol model/docs. | [patch] | ATLAS-MOIRAI-ORDERING-052 |
