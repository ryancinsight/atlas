...
| FWI-024-B | Cap the NLCG β with Fletcher–Reeves: `β = min(max(β_PR,0), β_FR)` (Gilbert–Nocedal). | [patch] | **review** — kwavers PR #406, commit `500250b40` | Claude | Convergence on the existing inversion tests is monotone and no worse than `β_PR⁺`; a case where unbounded `β_PR` overshoots is added as a regression test |
...
| US-023-D4 | Move `block_matching` out of `ritk-registration` into a dependency-light crate; `ritk-registration` re-exports it. The module imports only `anyhow` and takes `&[f32]`+dims, but its current crate pulls ritk-image/coeus autograd/nn/wgpu, which no consumer of the kernel needs. Gates D3. | [arch] | todo | — | The new crate gains no dependency beyond `anyhow`; ritk-registration's suite stays green; kwavers can depend on it without enabling `clinical-imaging` |
| US-023-D3 | Consolidate kwavers' NCC + parabolic speckle-tracking kernel (`.../elastography/thermal_strain/tracking.rs`) onto the block-matching seam and delete the duplicate. | [minor] | blocked: depends on US-023-D4 — the seam currently lives behind `ritk-registration`'s heavy stack, and `tracking.rs` is not feature-gated (`gap_audit.md#atlas-d3-placement-034`) | — | kwavers' tracking tests pass through the seam; no second NCC peak-refinement implementation remains |
...
| US-023-A4 | `SliceSeries` variant on the ADR 0042 seam. Design settled by **ADR 0047**: owned per-slice rigid transform list (memory budget is three orders below the image it describes), composition with `Direction`, forward clamp / inverse reject out of range. `CoordinateMap` stops being `Copy`, so it lands as a breaking change. | [arch] | todo — design settled, implementation open | — | Round-trip within a stated interpolation bound; a pure-translation sweep reproduces the Cartesian map exactly; single-slice degenerate case behaves as the 2-D map |
