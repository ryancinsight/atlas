...
- **Consumer half delivered 2026-08-04**: kwavers PR #349 deletes
  `kwavers-optics` and rewires `kwavers-phantom` (5 files),
  `kwavers-diagnostics`, the facade manifest, the crate-list doc, the
  photoacoustic example and `architecture_boundaries`.
- Scope correction to my own earlier census: I reported "73
  `absorption_coefficient` call sites". That grep matched the same method name
  on unrelated types (`blood_props`, optical-property structs). The real
  database usage was **3 calls across 6 files** — the work was a fraction of
  what that number implied. Check the receiver, not just the method name.
- Two latent defects were dropped rather than carried over: `kwavers-phantom`
  fetched extinctions with `.unwrap_or(0.0)`, so an out-of-range wavelength
  silently produced **zero absorption** rather than an error; and both
  consumers hardcoded `2.303` for ln(10) in duplicate Beer-Lambert
  conversions, which now happens once in hyperion with
  `core::f64::consts::LN_10`.
- Scoping question resolved: `typical_blood_parameters` **stays in kwavers**
  as named constants. Whole blood at 2.3 mM with 0.98/0.75 saturations is
  haematology, not an optical coefficient.
- **Reusable trap, worth its own note: the overlay hides stale first-party
  pins.** The rewire compiled and tested clean locally because the overlay
  resolves hyperion to the working tree, which already had the merged module.
  CI resolves from git, where the committed lock still pinned hyperion
  `4657996` — the pre-merge revision — so **25 checks failed at once** on a
  module that did not exist there. Fixed by `cargo update -p hyperion` run
  outside the overlay (`14e9de5b3`).
- The co-evolution rule that catches this: after an upstream merge, bump the
  consumer's pin in the *same* unit as the consumer edit. A local green build
  proves nothing about a cross-repo API addition, because the overlay is
  showing you a tree the lockfile does not name.
