...
- **Active claim (atlas coordinator):** Kwavers post-merge pointer, overlay,
  and lock-form recheck; scope is the merged provider default and Atlas-owned
  integration evidence. Peer-owned source, consumer, and lane checkouts remain
  out of scope.
...
| US-023-A6 | Decide whether B-mode moves behind the `kwavers` ritk bridge so scan conversion becomes a true `resample` through the seam and the converter is deleted outright. Splits the B-mode pipeline across crates, so it is a recorded decision, not an incidental one. | [arch] | **done 2026-08-19** — ADR 0048 Accepted; kwavers PR #412 in review: `ScanConverter` deleted, `scan_convert()` routes through `Image::physical_point_to_continuous_index` seam; 10/10 b_mode tests pass | Claude | Met. Differential oracle passes; no bespoke polar arithmetic remains in kwavers-analysis |
