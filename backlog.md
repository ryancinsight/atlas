...
- Evidence: Aequitas merged `8cc90b2`, PR #10; Kwavers `0bb44f70e`, PR #328; provider
...
- Owner: fable-prompt-session (claimed 2026-07-30). Claimed scope: `scripts/atlas-conformance.py`, `scripts/conformance-baseline.json`, this entry. Sub-delivery in flight: the scanner itself (scope 1), extended past the eleven recorded classes to the newly mandated unenforced rules (gitattributes/LF policy, nextest budget config, `[workspace.lints]` inheritance, sleep-synced tests, commented-out code, repo-local target forks, re-export shims). Burn-down (scopes 2-4) stays unclaimed for peers.
