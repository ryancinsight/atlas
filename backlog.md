...
| #7 | `[arch]` | CR-4: `coeus-core::Scalar` + `leto-ops::Scalar` rebase to eunomia supertraits (provider land) | (See CR-4 row above) | (None) | (See CR-4 row above) | `coeus/coeus-core/src/dtype/traits.rs` + `let''o/crates/leto-ops/src/domain/scalar.rs` now supertrait-bound; downstream chain patches (`cfd-math/src/linear_solver/chain.rs:62-72`, etc.); `cargo nextest run` green across `coeus, leto` workspace |
