...
- **Ratchet clear of file-size rows, 2026-09-02 19:30.** Every `oversized_files` and `manifest_implementation` regression is closed (athena, leto ×2, themis, kwavers, mnemosyne, apollo, hermes). The one remaining row is live-only and not a code defect: `ritk/excess_worktrees 0 → 1`. ritk has three working trees — its main plus two harness lanes under `.codex/worktrees/` holding `feat/tractography-dti-volume` and `feat/ritk-tractography-library`, both unmerged, last touched 17:11 and 17:17 UTC. They are their owner's to land or close; deleting another agent's unmerged work is not a bound fix. `worktrees/kwavers-log` remains an unregistered directory a peer process still holds open.
...
> **Active claim (2026-09-02, Codex):** execute the recorded upstream unblock in `repos/leto/crates/leto-ops`, `repos/hephaestus/crates/hephaestus-core`, `repos/hephaestus/crates/hephaestus-host`, `repos/hephaestus/crates/hephaestus-wgpu`, and the disjoint Coeus generic/provider integration surfaces in `crates/coeus-hephaestus` and `crates/coeus-wgpu`. Preserve existing vendor and peer-owned kernel edits; the acceptance oracle remains the staged ADR 0052 sequence and focused package gates.
