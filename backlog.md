...
| ATLAS-HERMES-AMX-CONFIG-087 | Hermes PR #40 `7526ee1` | Corrected the AMX irregular-width configuration and repacked row-major GEMM right-hand panels into the VNNI layouts required by the dot-product instructions: `K/4 × 4N` for INT8 and `K/2 × 2N` BF16 elements. The provider-owned packer covers fixed-tile and blocked-GEMM paths with independent value-semantic four-byte and two-element grouping tests. Follow-up cleanup closes Hermes `must_use_candidate` 162→0, `elidable_lifetime_names` 131→0, `missing_errors_doc` 83→0, `missing_safety_doc` 8→0, and `semicolon_if_nothing_returned` 92→0, splits the SIMD view-cast and `SimdOps` blanket-implementation leaves, scopes the macro's unreachable-code expectation to Neon with a regression test, rejects zero NTT moduli with a typed error, and canonicalizes NTT residues before subtraction. The current local Hermes/core/intrinsics nextest is 454/454, the Hermes/core doctest gate is 18/18 with 7 ignored, the benchmark-target smoke gate passes, and the targeted lifetime/error-doc/unit-statement audits are clean. Hosted run 31777497958 is running on `7526ee1`; all seven jobs are active with no failure reported yet. |
...
vs `2ea17bb`), Leto (`7f80044` vs `f3756b5`), Hermes (`81502c5` vs `9fdbd16`),
and Iris (`eec9818` vs `899d622`). Ritk, Eunomia, Gaia, and Tyche now match
...
lint/docs/structure cleanup at `7526ee1`; local contract gates are green, and
hosted run 31777497958 is running against that exact head. All seven hosted
jobs remain active with no failure reported yet. The prior benchmark and
Intel SDE AMX differential checks are green. The six
