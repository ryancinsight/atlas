...
| ID | Outcome | Class | Acceptance oracle |
| --- | --- | --- | --- |
| ATLAS-LINT-FLOOR-054 | **17 of 25 members have no `[workspace.lints]`.** Where a floor is declared it is then nullified: CFDrs correctly inherits `unwrap_used`/`print_stdout`/`print_stderr`/`dbg_macro` at deny in all 12 manifests, against **288 crate-level `#![allow]` and 5 `#[expect]` repo-wide, none with a ratchet reason** — which is why 402 library print sites survive a deny. coeus has 117 allow lines with **zero** `reason=`. | [patch] | **17 of 24 at `deny` as of 2026-08-14**, up from 0: leto, mnemosyne, moirai, harmonia, horae, hyperion, iris, proteus, aequitas, themis, melinoe, gaia, hephaestus, helios, coeus, ritk, plus consus already there. **Remaining: 7 at `warn`** (CFDrs, apollo, asclepius, athena, eunomia, hermes, tyche — a declared floor that does not fail a gate) **and kwavers with no pedantic line at all**, deferred because its census was blocked until `ritk-segmentation` compiled again (fixed in ritk `6ac3fb5e`). Two premises in the outcome column were wrong and are corrected here: several "missing `[workspace.lints]`" members are **single-package repos** where a package-level `[lints]` table is the only correct home, and the three original reference floors carry no `print_stdout`/`print_stderr`/`dbg_macro` lines, so "match that shape" and "add the print lints" conflicted — resolved per repo against measured production counts. Three mechanics worth reusing: a file-level `#![expect(clippy::unwrap_used)]` in a `src/` file whose unwraps are all under `#[cfg(test)]` **fails** as `unfulfilled_lint_expectations`, so those take `#![cfg_attr(test, expect(...))]`; a member with its own `[lints.rust]` table cannot also inherit; and adding `[lints] workspace = true` to members before the root table exists breaks manifest parsing for the whole stack through the overlay. The floors found real bugs, not just debt: a case-sensitive `.dll` match in hephaestus that silently missed `NVRTC64_120_0.DLL`, a reachable `Instant` subtraction panic in moirai's registry, a dead `let _z` in helios, and two `bool_to_int_with_if` ladders in mnemosyne. |
