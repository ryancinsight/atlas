...
- Result: all twenty candidates were ancestor extensions and every delta was
  ...
  `afab630b`, moirai `e3056e12`, ritk `c608f758`, melinoe `2d9696e9`, leto
...
- Merged after hosted checks passed: Themis #18, Proteus #8, Hyperion #8,
  Tyche #21, Mnemosyne #48, Aequitas #24, Asclepius #15, and Eunomia #62.
  Atlas reconciled their default heads in the current gitlink integration
  commits; the remaining caller PRs stay open pending their own required
  checks.
...
| US-023-A5 | Move `CoordinateMap`/`CurvilinearArray`/`PhasedArray3D` from `ritk-image` to `ritk-spatial` (pure `f64` geometry, no tensor coupling); `ritk-image` re-exports and keeps using them. Puts the geometry at the deepest common ancestor of its consumers and makes it reachable from `kwavers-analysis` without dragging coeus autograd/nn/wgpu into a DSP crate. | [minor] | todo — gates A3 | — | `ritk-spatial` gains no dependency it lacks; ritk-image/ritk-filter suites stay green; the seam's behaviour is unchanged (same tests, moved) |
