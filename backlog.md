...
| ATLAS-CFDRS-SCALAR-046 | **Stale — consolidation already done.** `rg 'trait \w*Scalar' crates/` returns exactly 1 hit (`CfdScalar` at `cfd-core/src/scalar.rs:352`). `from_f64` and `to_f64` duplication eliminated. | [minor] | Closed — oracle met on current tree |
