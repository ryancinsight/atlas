...
- **Down to one row, 2026-09-02 evening.** The apollo and hermes rows cleared as the owner's fixes landed and their gitlinks advanced; the ratchet's last regression is mnemosyne `oversized_files` 6 → 7, addressed by Mnemosyne#115 (`huge_pool.rs` 501 → 405 lines, the per-node bucket types lifted into `pool/node_huge_bucket.rs` and re-exported at their published path).
- **Evening burn-down wrap (2026-09-02, Freebuff session; atlas `067373ee5`, `6dd8398c9`).** Merged and advanced: apollo#288 (stft leto-reference sidecar, rebased past the peer lane-splits), mnemosyne#114 (free-path helper split — drops `free.rs` 534 from the oversized list), hermes#144's value-assertion wave recorded at its merged main. Baselines tightened (apollo `oversized_files` 41→39, `manifest_implementation` 37→25, `allow_sites` 21→19, `crate_level_allows` 34→30, gitattributes/timeout rows to 0; hermes `existence_only_assertions` 13→0 and `default_branch_cancel_in_progress` to 0; mnemosyne `allow_sites` 11→0, `crate_level_allows` 2→0, `toolchain_request_overridden` 1→0, `missing_deny_docs` 3→1) and one phantom floor corrected: mnemosyne `oversized_files` is **7, not 6**, at the recorded gitlink `088a0921` — the 6 was generated while the free-path split sat uncommitted in a dirty worktree and no pushed commit ever measured 6; six only becomes true once Mnemosyne#115 lands (`huge_pool.rs` 501→405), so the floor was restored to 7 with the per-file proof in the commit message. ritk `excess_worktrees` 0→1 is two live Codex sessions on unmerged tractography branches (`0704` at 1.7 h, `db6b` at 1.6 h since last write, 3 unmerged commits, no PRs) — the owner's to prune when those branches go up. Scanner probes confirm apollo `type_suffixed_fns` reads 55 at the tip (the transient 57 lived at an intermediate gitlink); the stft sidecar rename is invisible to the detector because `tests/` directories carry the test classification.
- **Apollo/hephaestus assertion burn-down + `_tests` detector (2026-09-02 late, Freebuff session; PRs apollo#292, hephaestus#258, both merged).** The four base128 dynamic-plan fallbacks assert the incumbent route round-trips within tolerance instead of asserting twiddles exist; the provider-audit rejection names its offending option; `BenchmarkSuite::emit` is gone from the library and its 14 call sites print `suite.report()` themselves. Detector companion committed in atlas scripts: `<name>_tests` directories (`lib_tests/`, `scalar_tests/`, `nn_*_tests/` — seven stack-wide) count as test paths; this tightened apollo `type_suffixed_fns` 55→49 and `unwrap_production` 193→132 and surfaced hephaestus's one real `workspace.is_some()` (burned in #258). Baselines: apollo `print_dbg` 4→0, `existence_only_assertions` 5→0; kwavers `unwrap_production` 262→261. The hephaestus `manifest_implementation +1`/`orphan_modules +6` rows belong to the owner's 4,214-line generic-window-backend wave merged alongside #258. Also fixed in passing: the pre-commit pin check resolved per-member refs through git's exported `GIT_DIR`/`GIT_INDEX_FILE` against the superproject, refusing valid hephaestus (`master`) gitlinks — hooks now unset them.
- **A recurring generator behind this session's gitlink churn, now guarded.** `git commit` without a pathspec records each staged submodule at its *worktree* HEAD, and in this shared checkout those checkouts sit on feature branches; a pathspec-limited commit does the same for the paths it names. Three commits today pinned apollo to `test/apollo-stft-leto-sidecar` and mnemosyne to `refactor/mnemosyne-free-helpers-split`, each caught only when the pre-push guard blocked an unrelated push later. The pre-commit hook now reads the *staged* entry and refuses the commit that creates such a pin (`a3369cc75`), which is the check the pre-push auditor cannot make because it reads HEAD. Verified both ways.

- **Detector precision + recorded-gitlink scan (2026-09-02, `1c6924003`, in progress: claude-fable session 5050c72a):** eight of the twelve rows the ratchet flagged at `a7d33bb3` were the detector — keyword-led prose counted as commented-out code (apollo 5→7, hermes 7→10 were all prose), `is_none()` counted as existence-only (apollo 1→8), and the test-region splitter cut at the first literal `#[cfg(test)]` so `#[cfg(all(test, windows))]` timing macros counted as library `eprintln!` (apollo print_dbg 4→10 with the comment-separated `#[cfg(test)] … #[cfg(not(miri))] mod retained_footprint;` sidecar). Detectors fixed with tests; the scan now reads each member at its recorded gitlink (archived snapshot when the checkout is behind or dirty — CFDrs blocked every local scan today), so `generate` and `check` no longer need a clean stack. Baseline moved by exactly the detector delta (old vs new detectors on the same snapshots; consus tightens 399→25 unwraps because its `all(test, feature = "alloc")` test module had counted as production). **Remaining real regressions (atlas conformance stays red until burned down — the ratchet's contract; never `--accept-raises` for code):**
  - `oversized_files` (>500 lines; newest crossings at the recorded gitlinks): athena +1 `athena-core/src/solver/lsqr/algorithm.rs` (596); coeus +1 (08-26 batch: `coeus-autograd/src/{autodiff_cache.rs 1021, backward_cache.rs 721, gradcheck.rs 593, ops/linalg.rs 557}`); leto +1 (`leto-ops/src/domain/scalar.rs` 636 or `leto-ops/tests/ops/elementwise.rs` 552, both 09-02); themis +2 (`src/topology/cpu/mod.rs` 523, `tests/topology/cpu.rs` 514); mnemosyne +2 (09-02: `mnemosyne-local/src/tests.rs` 1010, `local_alloc/tests/cross_thread_tests.rs` 917, `mnemosyne-arena/src/segment/tests.rs` 796, `mnemosyne-local/src/free.rs` 534, `mnemosyne-core/src/types/segment.rs` 524, `segment/pool/huge_pool.rs` 501); hermes +6 (09-02: `hermes-simd/tests/kernel_property_tests.rs` 1769, `hermes-simd-core/src/kernel/backend.rs` 1490, `view/vector_reg.rs` 1049, intrinsics `avx2_f32` 879 / `avx512_f32` 852 / `avx512_f64` 818 / `avx2_f64` 781 / `neon_f32` 717 / `neon_f64` 664); CFDrs +1 (08-26 batch, 139 files over the target — split campaign of its own); apollo +9 (09-01/02: `plan/fft/dimension_1d/plan_tests.rs` 1368, `components/batched/mod.rs` 1065, `benches/engine_census.rs` 908, `base128/tests.rs` 731, `dynamic_impl.rs` 674, `retained_footprint.rs` 648, `small_pot/reduced.rs` 521, `mixed_radix/scalar/impls.rs` 624, `small_pot/precise.rs` 593, `stockham/butterfly/{fixed.rs 566, stage.rs 518}`, `base128/instance_major.rs` 521).
  - `manifest_implementation` (implementation in `lib.rs`/`mod.rs`): apollo +6 (`components/batched/mod.rs` 553 body lines, `winograd/composite/mod.rs` 44, `transport/cuda/mod.rs` 21, `components/{base128 207, stockham 206, rader 184, good_thomas 163, codelet 93, butterflies 43}/mod.rs`, `apollo-bench/src/measurement/mod.rs` 42); kwavers +1 (09-02: `kwavers-math/src/fft/mod.rs` 407, `kwavers-python/src/simulation_py/mod.rs` 108, `kwavers-gpu/src/gpu/mod.rs` 91).
  - Detector-visible now that the splitter is item-scoped (production code after an early sidecar declaration): ritk `allow_sites` 0→22 (16 at the old detector — a real regression of 16, plus 6 unmasked), `type_suffixed_fns` 67→72, kwavers `unwrap_production`/`allow_sites`/`print_dbg` and moirai `unwrap_production` moved by their detector deltas only; their code regressions stay flagged.
  - **Landed (2026-09-02, integrator: claude-fable session 5050c72a):** athena#28 `93e428f4`, leto#149 `eb0ff478`, themis#44, kwavers#696, Moirai#243 — every gitlink advanced (`8b6217f3c` latest). Baseline measured at `7a0916863`'s recorded gitlinks (`91c190388`), so the ratchet now fails only on what landed after that revision. **Remaining rows (peer-owned; the owners' active files):** apollo `cook_toom_gt.rs` 495→505 and `apollo-stft/.../dimension_1d/tests.rs` 465→504 (the only oversized crossings since 7a0916863 stack-wide), apollo `print_dbg`/`existence_only` real sites, mnemosyne `free.rs` 498→534 and `huge_pool.rs` 499→501 (unsafe allocator hot paths, no inline tests to sidecar — the owner's design), ritk ten `#[allow(…, reason = "ratchet RITK-LINT-1")]` sites (the lint floor's form is `#[expect]`; owner's item RITK-LINT-1), moirai `commented_out_code` 0→1. Lane skeletons `worktrees/kwavers-pstd-backend` and `worktrees/moirai-value-assertions` are unregistered but held open by another process (same class as `worktrees/kwavers-log`; delete when the handle releases).
  - **Burn-down (2026-09-02 afternoon, integrator: claude-fable session 5050c72a):** athena#28 `93e428f4` (LSQR driver split, landed; gitlink advanced `eff2d54f9`); leto#149 `eb0ff478` (Scalar trait/impls/fallbacks split, landed; gitlink `566ba8bc4`); kwavers#696 (fft facade manifest → leaf modules, enqueued), themis#44 (CpuTopology out of the cpu manifest + test harness split, enqueued), Moirai PR (ten existence-only assertions → panic-message and value assertions, `test_support::panic_message` consolidating two inline downcasts, enqueued). Baseline re-anchored at `7a0916863`'s recorded gitlinks (`0a5dc21be`; a second pass against the pre-change file `9ca66f3c3` corrects the apollo print/existence rows the first pass clamped to 0). Remaining after these land: apollo (oversized +9, manifests +6, print_dbg 4→? real, existence 1→5 real), hermes +6 oversized, mnemosyne +2, coeus +1, CFDrs +1, ritk `allow_sites` 12→22 (ten real `#[allow(unused_imports, reason = "ratchet RITK-LINT-1")]` sites — `#[expect]` form per the lint floor, or the imports fixed), moirai `commented_out_code` 0→1.
  - **Burn-down (2026-09-02 evening, integrator: Freebuff session):** moirai `commented_out_code` 0→1 → Moirai#246 (the single site was the stale `// pub mod hybrid; // Removed: …` declaration in `moirai-core/src/lib.rs:47` — the module never existed in this crate's tree and the fact it records is stated by the README crate table; gates: fmt, warning-denied clippy, nextest 99/99). Moirai#245 records the `MOI-PAR-RECURSIVE-DRIVE-2026-09-02` disposition as **rejected** with paired Criterion evidence (map/reduce +178%, count/min/max +502%, early find +500%, missed find +254% at 131,072 items — per-level `split_off` copy traffic doubles under full binary recursion and each level's `global().scope`/claim-mutex overhead dominates; CHECKLIST.md updated on that branch, no implementation ships). Next increments, closest-to-done first: athena `lsqr/algorithm.rs` split (one file), themis two files, leto one, kwavers `fft/mod.rs` manifest (move the 407 body lines into leaf modules), then mnemosyne, hermes, apollo, coeus, CFDrs as per-member items.
...
- **Burn-down (2026-09-02):** apollo `gitattributes_missing` + `workflow_missing_timeout` → apollo#270 (`.gitattributes` on the stack template; `timeout-minutes: 15` on `python-release.yml`'s `publish` job — the `wheels` job is a reusable-workflow call, which GitHub forbids a timeout on and the rule already exempts); athena `tag_pinned_actions 0→9` → athena#22 (nine `uses:` pinned to the SHAs six other members run; `toolchain: 1.97.0` stated instead of the action ref `@1.95.0`, which had installed a toolchain two minors behind athena's own `rust-toolchain.toml`). Later the same day: athena `existence_only_assertions 0→1` → athena#27 merged (the one `assert!(outcome.is_err())` in `preconditioner_contract.rs` now reads the error and requires both mismatched lengths in its message — the seam's error names them, so the contract holds as a value). hermes `commented_out_code 7→10` reads 0 with the scanner's pattern at `origin` — the counted sites live in peers' unlanded trees at the gitlinks the run scanned, so that row closes when those trees land clean or their authors fix them; the remaining rows are code debt in members with live peer activity (apollo, moirai, ritk, kwavers, coeus, themis, CFDrs) and belong to their integrators.
...
- **Current gate verdict (2026-09-02), claude-sweeper session:** latest
  default-branch `atlas-conformance` runs (`970adf36` run 33651868542,
  `81598f83` run 33653271413) both fail with the same three real regressions,
  all in peer-owned members whose lanes this session cannot touch:
  `apollo/oversized_files` 41->42, `apollo/lane_kernel_uninlined` 3->4
  (both from the two-digit Stockham advance), `mnemosyne/oversized_files`
  6->8. The `~50` tightenings they also list (mostly
  `default_branch_cancel_in_progress` ->0 as fixes landed) are already
  recorded in the committed baseline (`99ae5b017`); the gate stays red until
  the apollo and mnemosyne integrators land their burns-down — never a
  baseline bump for the raises. Local live audits additionally over-count
  `target_forks` when stale repo-local `target/` caches sit in member
  worktrees; cleaned the seven such caches found today, so a dirty-tree scan
  now matches a clean checkout.
...
- **Two adoptions needed correcting, both for reasons that were not semver verdicts.** CFDrs failed on all eleven crates with `failed to run custom build command for yeslogic-fontconfig-sys` — cargo-semver-checks builds rustdoc for both revisions, so a crate whose build script links a system library fails here exactly as `cargo doc` would, and the shared job installed nothing beyond the toolchain while CFDrs's own CI does. The gate gained an `apt-packages` input (`fcbc5202c`, empty by default) and CFDrs passes `libfontconfig1-dev`. Coeus reported `no semver update required` for fourteen crates and failed on the fifteenth: `coeus-cuda`'s baseline cannot resolve `cuda-async ^0.2.0` on a hosted runner, so it is dropped from the checked set — a crate whose baseline needs a vendor SDK cannot be compared where the SDK does not exist. Both branches rebuilt on current main.
- **Second and third catches, 2026-09-02.** Validation dispatches of the newly gated release workflows: **iris** passes clean (`iris-viz` 0.1.0 covers its surface). **leto** fails with five lint classes against its published `leto` 0.42.0 — `inherent_method_missing`, `module_missing`, `struct_missing`, `struct_marked_non_exhaustive`, and `struct_pub_field_missing` (`Layout::shape`, `Layout::strides`, `Layout::offset`) — while its manifest still reads 0.42.0. leto has accumulated a major API revision since its last publish, and the gate now stops that release rather than shipping it under an unchanged version. Same resolution as gaia: a 0.x breaking change needs the minor bumped at release, which is the owner's release decision, so no version moves here. ritk's dispatch is still queued.
- **Fourth catch, 2026-09-02: ritk.** Twenty-nine of its thirty published crates report `no semver update required`; `ritk-filter` fails `constructible_struct_adds_field` — `MarchingCubesFilter.direction` (`crates/ritk-filter/src/surface/marching_cubes.rs:64`) is a new public field on a struct that was exhaustively constructible, so existing struct literals break. Same shape as gaia's, same resolution (the owner's version bump at release).
- **Consolidation, 2026-09-02:** asclepius#37 and proteus#28 replace the hand-rolled `cargo-semver-checks` jobs those two carried from before atlas owned the gate. All 24 members with publishable crates now call the one workflow; no per-repo copy remains, which is the shape the item specified (ADR 0035).
- **The gate caught a break in this session's own refactor.** mnemosyne#115 moved `NodeHugeBucket`/`NodeHugePool` out of the public `pool::huge_pool` module to bring a 501-line file under the target, and the informational job reported `struct_missing` before the merge. The types are re-exported at their original path; a file-size refactor may not change the public surface.
- **Versions bumped on the user's authorization, 2026-09-02 evening.** gaia-mesh 0.4.0 → 0.5.0 (`d7d2f8b3`), leto 0.42.0 → 0.43.0 (`0d3b42ac`, workspace version plus two path requirements and three lock entries), ritk-filter 0.3.0 → 0.4.0 (open, blocked on the apollo fix below). Each carries a `BREAKING CHANGE:` footer naming what moved; no code changed, the versions now cover the surface the code already had.
- **The leto bump is a five-wave co-evolution, not one commit.** A git dependency without a rev resolves to the branch head, so the moment leto's main read 0.43.0 every consumer's main stopped resolving from scratch (`^0.42.0` admits only 0.42.x); their committed locks still pass `--locked`, which is why nothing looked broken. Cargo resolves one leto version for the whole graph, so consumers move in dependency order: **1** gaia, hephaestus (merged `4a78d77f`, `f00175fe`); **2** apollo, athena (merged `3cafb06b`, `4d549779`); **3** coeus; **4** ritk; **5** CFDrs, kwavers. Each wave's lock must also name the previous wave's revisions, since a sibling still locked at its pre-sweep revision keeps requiring the old upstream.
- **Locks are regenerated by cargo, never edited.** The first attempt hand-edited the version and revision strings and `--locked` rejected it (gaia#43). `regen_lock.py` (scratchpad) builds a resolution skeleton — every `Cargo.toml` at `origin/HEAD`, the lock, and an empty target file per declared target — in a temp directory and runs a source-qualified `cargo update` there, so cargo writes the lock and no shared tree is touched. Three defects in that tool were caught by its own post-condition: it advanced only leto while a sibling's pre-sweep revision still required 0.42.0; its crate pattern was unanchored, so `leto` rewrote `athena-leto`'s version; and bare `-p` specs are ambiguous where a lock holds one crate from two sources (coeus's rev-pinned `mnemosyne-arena`).
- **Upstream defect found by the sweep:** apollo gated its lane kernel module on `x86_64` but imported those kernels unconditionally in two precision modules, so every non-x86 build failed at the `use` (E0432). apollo's CI is x86 Linux only; ritk's macOS wheel job had been failing on it since before this work. Fixed in apollo#294 (`72bb471f`).
- **The shared gate's informational job fails on every consumer during a sweep** — it builds the *baseline* revision, and that revision pins the upstream the sweep is moving away from. It is `continue-on-error` by construction, so the merge waiter gained `--advisory` (`8f4dd1e0`-class change, tests pinned) rather than a hand merge.
...
- [x] **ATLAS-WINDOW-ACCELERATOR-INCOMPLETE-2026-09-03** [arch][patch] status=done
  ...
  **Scope:** hephaestus's spatial-window accelerator layer is declared but no backend implements it, and Coeus is already
  ...
  `D::CacheKey: From<WindowKey>` in `launch.rs`, `pooling.rs` and `sliding.rs`, and no backend provides that impl —
  `hephaestus-cuda` has `From<AxisScanKey> for CudaKernelCacheKey` and nothing for `WindowKey`; (2) `coeus-cuda`'s
  `pool.rs` and `unfold_fold.rs` import `super::window::WindowConfiguration`, but `window.rs` and its `mod` declaration
  exist only under `coeus-wgpu`; (3) those same two files import `crate::backend::CudaBackendError`, while the type is at
  `crate::CudaBackendError`. **Non-goals:** changing the window layer's design; adding a second `WindowConfiguration`
  copy — it is already in two backends, so its home is the crate both depend on. **Acceptance:** the four provider-contract
  jobs compile and pass on Coeus main. **Dependencies:** the `From<WindowKey>` impls land upstream in hephaestus first,
  then Coeus's lock advances to them. **Found:** 2026-09-03 while merging Coeus#361, whose own scope was orthogonal.
  **All three causes fixed and merged, 2026-09-03 (claude session 5050c72a) — the lease above was taken after this
  work was already in flight, so codex should check before starting: nothing of the three remains.** hephaestus#263
  (`89e50a9e`) adds `From<WindowKey>` for the CUDA and ROCm cache keys; metal and wgpu implement no `DeviceApi`, so
  they carry no key to extend. Four tests assert that a difference in operation, host scalar or spatial rank yields a
  different key, and that the same window converts identically. Coeus#362 (`665f715a`) moves `WindowConfiguration`
  into `coeus-hephaestus` rather than copying it into the second backend, names `CudaBackendError` where it lives,
...
- **Apollo carries two commits reachable only from a detached HEAD.** `repos/apollo`'s working tree sits detached at
  ...
  the local branch `chore/apollo-mnemosyne-rev-and-f16-gate` so garbage collection cannot reap them. It will not push:
...
| ID | Outcome | Class | Acceptance oracle |
| --- | --- | --- | --- |
| ATLAS-COEUS-BACKEND-045 — **closed 2026-09-02** | Coeus pooling and unfold/fold now use the generic Hephaestus provider seams for WGPU (`eb09679a`) and CUDA/Metal/ROCm (`d9c567c6`); superseded Coeus CUDA window kernels are deleted. Hephaestus registers and re-exports the CUDA, Metal, and ROCm generic window providers and fixes the C-family zero-sized-type `Default` bound (`23810ae`). Leto supplies validated pooling and sliding-window contracts (`ce3a178`, `c4d6512`). Coeus uses Eunomia for the direct reduced-precision conversion (`02e3b319`). | [arch] | Leto `leto-ops` nextest 197/197 and clippy pass; Hephaestus core/CUDA/ROCm nextest 154/154 and clippy pass; Coeus CUDA/Metal/ROCm focused clippy passes and native nextest 1/1 passes; Coeus WGPU nextest 142/142 was green on the preceding provider slice. Coeus direct `half` dependency/import search is empty; remaining `half` is transitive through Criterion/Naga. CUDA-feature compilation remains blocked by missing `D:/msys64/ucrt64/bin/libclang.dll`; no hardware-backed CUDA/ROCm/Metal execution was available. Existing direct-WGPU residuals in other operation families/repos remain tracked separately. |
...
- **ATLAS-COEUS-PARITY-ALLOC-BUDGET-2026-09-02** coeus `main` is red on `Backend parity`: `repeat_interleave_allocation_count_is_independent_of_output_size` fails only in the job that builds coeus against every first-party default-branch tip. It passes locally against coeus's pinned lock **and** against the same tips through the stack overlay, on Windows — so the trigger is that job's Linux/wgpu configuration, not the pinned versions. An allocation-count oracle is exactly what a global-allocator change perturbs, and athena#30 ("Mnemosyne as global allocator in allocation contract tests") is that campaign in flight; left to it rather than duplicated. Reopen if the campaign lands without clearing the red [patch] — status: filed, unclaimed (2026-09-02)
