...
- **Done:** all 57 workflows across 26 repositories carry the per-commit/pull-request-only form (verification) or `cancel-in-progress: false` (deploys); the conformance class reads 0 at every member's `origin` and the collector shows no cancelled default-branch row. Landed through waves 1, 2a, 2b, 2c, 2d (deploy classification by file name; CRLF-tolerant anchors) plus atlas's own two workflows; the guard template no longer mints the class.
