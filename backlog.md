...
The two additional object/range cells required by P5-A remain outstanding, so
P5-A is baseline-passing but not fully qualified. P5 remains explicitly staged:
MinIO qualification first, then a next-breaking-release native default only if
every benchmark cell reaches at least 90% of legacy median throughput with
correctness intact, followed by a one-release legacy compatibility window
before Tokio/Rusoto retirement. No default flip is claimed from the baseline
cell alone.
