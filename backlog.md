...
| ATLAS-ADR-GOV-058-APOLLO | **In progress — claimed 2026-08-14.** Normalize Apollo ADR status headers and regenerate its derived index from the current provider corpus. Preserve supersession, dated acceptance, and open implementation status as revision notes rather than flattening them into false completion. Acceptance: Apollo has no generator anomalies or index drift, provider exact-head gates pass, and the Atlas gitlink advances only after merge. The peer-owned performance branch and dirty lockfile remain outside scope. | [patch] | ATLAS-ADR-GOV-058 |
