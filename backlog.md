...
| US-023-D4 | Move `block_matching` into a dependency-light crate; also parameterize its sample type so `f64` RF consumers need no narrowing conversion. | [arch] | **done 2026-08-19** — ritk PR #183 merged at `038099040`; PR #184 (zero-flux pad split) also merged | Claude | Met. `cargo tree` shows exactly one edge (`anyhow`); 9 tests incl. cross-precision agreement and the 1-D line case; ritk-registration 375 tests still green |
| US-023-D3 | Consolidate kwavers' NCC + parabolic speckle-tracking kernel onto the block-matching seam and delete the duplicate. Shape maps cleanly (`dims=[1,1,nz]`, radius on the fast axis) and the precision mismatch is resolved by D4's generic sample type. | [minor] | **review** — kwavers PR #408 `feat/kwavers-d3-block-matching-seam` | Claude | Met. kwavers-physics 1551/1551 tests pass through the seam; no second NCC or parabolic-peak implementation remains in the stack |
