...
| ATLAS-CONSUS-SHUFFLE-038 | **HDF5 shuffle-filtered data is returned silently un-un-shuffled — wrong values, no error.** `dataset/chunk.rs:311` matches filter ID 2 (shuffle) and returns `Ok(data)` **unchanged**, deferring the inverse transform to "a higher-level reader" that does not exist. So a shuffle+deflate dataset decompresses correctly and is then handed back with its byte planes still transposed. Observed on `h5py_shuffle_deflate_i32`: returned `[50462976, 117835012, 0, 0, 0, 0, 0, 0, 185207048, …]` against an expected `0..15` — those decode to bytes `00 01 02 03 04 05 06 07` followed by 24 zeros, which is exactly the shuffled plane layout. **No error is raised**, so a consumer gets plausible wrong numbers rather than a failure. Found while re-verifying `-036`/`-037`; it is **not** the v1 B-tree descent bound, which applies to *group* trees while the chunk path separately rejects `header.level != 0` as `UnsupportedFeature` — neither fires, and both chunks were located and assembled in the right order. Deflate itself is fine: every pure-deflate case passes (1d/2d/3d, level 1, level 9, chunked f32). Note the workspace baseline is **1 failure, not the 7 previously recorded**. | [major] | The inverse shuffle is applied for filter ID 2, or the path returns a typed `UnsupportedFeature` — silently returning transposed bytes is not an option. Element size is available from `ds.datatype` and `consus-compression` already documents the transform. `h5py_shuffle_deflate_i32` passes with exact values, and a round-trip property test over several element sizes accompanies it. |
...
| RITK-CI-1 | Restore SimpleITK parity for `InverseDisplacementField` 2-D/3-D and `IterativeInverseDisplacementField`; the 2-D case is off by 2.23 against a 1e-4 gate. Start from main's recent rewrite of `inverse_displacement.rs` and `iterative_inverse_displacement.rs`. | [major] | todo — not mine; ritk main is red | — | The three `test_simpleitk_cmake_data.py` parity tests pass |
| RITK-CI-2 | `Test Suite (ubuntu-latest)` exceeds the 30-minute job cap during "Install LLVM and Clang (Linux)", so Linux tests never run. Cache or remove the LLVM install rather than raising the bound. | [patch] | todo | — | The ubuntu job runs tests and completes inside its budget |
| US-023-D2 | Block-matching follow-ons: multi-resolution search-region sources and block-radius calculators, FFT-accelerated NCC, Bayesian-regularized and strain-window displacement calculators, and an end-to-end pipeline over a block grid. | [minor] | todo | — | Each recovers a known field on a simulated compression sequence within a derived bound |
| US-023-D3 | Consolidate kwavers' NCC + parabolic speckle-tracking kernel (`.../elastography/thermal_strain/tracking.rs`) onto the ritk block-matching seam and delete the duplicate. | [minor] | blocked: depends on US-023-D landing | — | kwavers' tracking tests pass through the seam; no second NCC peak-refinement implementation remains in the stack |
...
| US-023-B2 | QUS increment 2: spectral-difference attenuation estimation (dB/MHz/cm) from the depth-rate of the reference-normalized spectrum. **Not ported from ITK**: `itkAttenuationImageFilter::ComputeAttenuation` fits a *ratio* of spectral components, which is dimensionally sound only for linear power, while `itkBackscatterImageFilter` treats the same input as dB. Implement from Yao/Zagzebski/Madsen (1990) instead and record the discrepancy. | [minor] | todo | — | Recovers a known attenuation from a synthesized two-depth phantom within a derived bound; the ITK divergence is recorded in `gap_audit.md` |
...
| US-023-D | Block-matching elastography framework — **increment 1**: metric-image and displacement-calculator seams, direct NCC, and max-pixel / parabolic / cosine refinement. | [arch] [minor] | **review** — ritk PR #173, commit `d758e2e3` | Claude | Met. Exact integer translations recovered exactly; half-voxel shift lands strictly between integers and beats the integer estimate; gain/offset invariance asserted. 382 tests, clippy, fmt clean |
