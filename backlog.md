...
| US-023-A6 | Decide whether B-mode moves behind the `kwavers` ritk bridge so scan conversion becomes a true `resample` through the seam and the converter is deleted outright. Splits the B-mode pipeline across crates, so it is a recorded decision, not an incidental one. | [arch] | **done 2026-08-19** — ADR 0048 Accepted: resample through CurvilinearArray seam; ScanConverter deleted | Claude | Implementation is the next PR; differential oracle within bilinear interpolation error |
