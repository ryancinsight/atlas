...
| ATLAS-MOIRAI-ORDERING-052-REACTOR | Audit and reduce the PAL reactor's three `running` `SeqCst` operations to the weakest flag protocol: the flag controls loop termination only, while `stop()` separately wakes the platform poller. Acceptance: no unrelated data publication relies on the flag; the reactor stop path remains value-correct under the existing executor/network tests; hosted provider gates pass. Scope: `moirai-pal/src/reactor/core.rs` and its focused tests/docs. | [patch] | ATLAS-MOIRAI-ORDERING-052 |
...
| ATLAS-MOIRAI-ORDERING-052 | Baseline audit found 624 production atomic sites with 10.1% carrying an ordering justification and high-strength ordering concentrated in executor/MPMC paths. The MPMC waiter model landed in `2ea17bb`; the SPSC ring model and hosted clean-checkout gate landed in `ac111b3`; the async executor wake-dedup model and Relaxed `is_queued` demotion landed in `fd517fe`. The remaining residual is the broader justification ratchet and production `SeqCst` reduction; no claim of closure is made. | [patch] | Justification coverage ≥ 90%; production `SeqCst` ≤ 20; the MPMC, SPSC, and async wake-dedup Loom models remain green with stated bounds |
