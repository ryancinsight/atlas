...
| ATLAS-ADR-GOV-058-RITK | Normalize Ritk ADR status headers and regenerate its derived index. ADR 0002 currently claims a completed Burn→Coeus cutover while the live tree still retains the Burn production path; preserve that technical fact in the status record. Acceptance: Ritk ADR index generator reports no Ritk anomalies or drift; provider gates pass. | [patch] | ATLAS-ADR-GOV-058 |
