...
| ATLAS-RITK-TRANSFORM-DIRECTION-081 — **closed 2026-08-18; the defect was worse than filed** | **They do not assume the identity direction — they hard-code the axial permutation** (index axis 0→z, 1→y, 2→x), pairing `spacing[2]` with world x. In this crate's convention the identity direction sends *depth* to world x, so the old behaviour was correct only for a conventional axial volume and wrong for everything else — **including the identity direction their own fixtures used**, which is exactly why no existing test caught it. Held at **7 of 8** sites; `csd.rs:1002` **did not hold and is correctly direction-free** (`FodVolume` carries no direction, has no production constructor, nothing upstream holds an `Image` — there is no matrix in scope to drop). The count read as 8 rather than 4 distinct bugs because `iterative_inverse_displacement.rs:231,233` is a byte-identical copy of `:75,77` and `inverse_displacement.rs` has the same pair — the beside-build duplication. **Two pieces of guidance in this row were wrong:** transpose is unsafe here, because `ritk-spatial` never validates orthonormality so non-orthonormal directions are representable and a transpose would be a silent scale error — `try_inverse()` is used, as everywhere else; and the ADR 0018 seam could not be called directly, since `physical_point_to_continuous_index` recomputes a Gauss-Jordan inversion per call, ruinous inside a per-voxel coordinate descent. `CartesianGridGeometry` is that mapping with the inverse hoisted, consolidating four hand-rolled versions. **Falsification:** reverting gives 6 oblique-test failures with **every axis-aligned test passing bit-identically**. Two adjacent defects fixed without which the change is meaningless — ritk-snap passed tensor-order spacing into an `(ix,iy,iz)` parameter, transposing x and z, and the TPS indexed `origin` by tensor axis rather than LPS component. Landed ritk `3aa73ba0`; nextest 5317/5306 baseline; ADR 0020. | [major] | Met, with `csd.rs` documented rather than changed. | Found inside ATLAS-RITK-VIEWS-047 while counting duplication: of 17 index↔physical transform implementations across 22 sites, **8 are direction-free** — `marching_cubes.rs:161`, `csd.rs:1002`, `iterative_inverse_displacement.rs:75,77,231,233`, and `inverse_displacement.rs:137,334`. They compose origin and spacing but drop the direction matrix, which is the identity only for axis-aligned acquisitions. Clinical CT and MR are routinely oblique, so these produce plausible, silently displaced geometry rather than an error. This is a **correctness** defect that was hiding inside a consolidation item, and it outranks the consolidation; ADR 0018 already unified the single-point pair, so the seam to route them through exists. | [major] | Every index↔physical transform applies the direction matrix, or documents in its own Rustdoc why its caller guarantees an axis-aligned volume. One regression test per fixed site uses a deliberately **oblique** direction matrix and asserts against a hand-computed physical coordinate — each must fail on the parent commit, since an axis-aligned fixture cannot distinguish the two implementations. |
