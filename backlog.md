...
| ATLAS-HERMES-AMX-CONFIG-087 | Hermes PR #40 `c693f53` | Fixed the AMX irregular-width defect: B/C configuration now clamps to the kernel's 16-column microtile, keeping the 1024-byte tile budget. Local `hermes-simd-intrinsics` nextest is 28/28, core nextest is 16/16, source `allow_attributes` fell 26 -> 0, and Atlas conformance reports Hermes `allow_sites` 30 -> 0. A follow-up removes four dead terminal AVX2 propagator assignments and seven target-specific unfulfilled expectations caught by hosted CI; first-party checks are pending on `c693f53`. The remaining HS-435 classes remain an independent provider blocker. |
